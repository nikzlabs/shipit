...
export function composeReviewMessage(filePath: string, opts: ReviewComposition): string {
  ...
  if (opts.mode === "role") {
    lines.push(
      ...
      "",
      "Name the ROLE and nothing else: no --agent, no --service, no --billing-mode.",
      "Pass through --model and/or --effort only for values the user named in this",
      "request (a specific model, a reasoning level); add nothing yourself. ShipIt",
      "picks the reviewer from its own settings — the one furthest from you that this",
      "install can run — so do not reason about which backend is far from you, and do",
      "not substitute a backend you happen to know is installed.",
      "",
