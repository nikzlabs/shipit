...
Dismissal hides **one failure, not the feature**: it is client-local state (nothing server-side changes, so a reload shows the banner again while the state is still exhausted), and it re-arms the moment the status leaves `"exhausted"`. That re-arm is correct because `"exhausted"` is terminal until user activity or a new head SHA resets the manager — so any later exhaustion is necessarily preceded by a non-exhausted status. It is written as a render-phase state adjustment derived from the status, not a `useEffect` (the client lint rule restricts effects).
