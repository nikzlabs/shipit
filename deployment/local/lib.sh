...
shipit_refresh_tailnet_bind() {
  ...
  if [ -z "$ts_ip" ]; then
    ...
    echo "    Tailnet access resumes automatically next start once Tailscale is up." >&2
    return 0
  fi

  # Written atomically so a concurrent `docker compose` never reads a partial
  # file. Compose concatenates `ports` across -f files, so this ADDS a binding
  # rather than replacing the loopback one in compose.yml.
  local tmp
  tmp="$(mktemp "${TAILNET_COMPOSE_FILE}.XXXXXX")"
  cat > "$tmp" <<EOF
