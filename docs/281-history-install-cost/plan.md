...
## Finding 1 — there is no truncation window, and the ordering is guaranteed (req 1)
...
Two things follow. The window is **closed**, and it is closed by design rather
than by luck — `useMessageHandler`'s queue exists for exactly this. And **none of
it is specific to the `304`**: the `200` path reaches the same install by the
same route with the same guarantee.

The one link that was not pinned end-to-end on the cached path is the last two
rows of that table, because a `304` reaches the install without parsing a body
and so takes a different code path to get there. That is now a test (req 5).
...
is pre-existing, identical on a `200`, and healing it means making the install a
merge rather than a replace — which is the question planning#268 (windowed
history loading) re-opens on its own terms.

## Finding 2 — the seeds correct a different channel, and must be unconditional (req 2)
...
as `pending` — actionable, Approve/Deny live — and only the authoritative seed
restores the persisted phase.

So "the transcript is unchanged" says nothing at all about what the replay just
wrote into the card stores. The `304` is not a licence to skip them (req 3), and
...
## The change: the cost, not the condition (req 4)
...
- **Switch-back** — the array was cleared, so the install genuinely runs; the
  rows are the same objects, so every row bails out of its memo.
