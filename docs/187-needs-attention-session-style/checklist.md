...
- [x] Implement in `SessionItem` (`SessionSidebar.tsx`) — center-origin `linear-gradient` (transparent → amber → transparent), driven by `--color-attention-wash`
- [x] Center-origin gradient (fades out on both sides) instead of left-edge
