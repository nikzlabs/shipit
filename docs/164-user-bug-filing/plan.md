...
- `filed`/`failed` transitions persist via `persistBugCardTransition`
  (`bug-report-handlers.ts`). The naive path — a direct
  ...
  `persistBugCardTransition` mirrors the permission-card path (`docs/193`): when
  `runner.running`, it patches the recorded card in place via
  ...
  `recordedCards` are inert → safe). Regression: `user-bug-filing.test.ts`
  "(b/d) keeps a filed transition through finalize when the proposing turn is
  still in flight".
