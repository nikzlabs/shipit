...
| `orchestrator/service-manager.ts` | 1484 | Collaborators (Poller/SecretsResolver/RetryManager) are already split, but the class still owns both `docker compose` CLI invocation and the start/stop/reconcile state machine plus install-gate tracking. | `compose-cli.ts` (compose invocation, conflict recovery, output parsing), keep reconcile state machine + collaborator wiring in `service-manager.ts`. Optionally `install-gate.ts`. |
