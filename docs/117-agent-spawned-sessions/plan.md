...
3. **Get the child's workspace.** When the parent's repo is registered in `repoStore` and ready (the production happy path), the spawn flow calls `claimSessionService.claim(parent.remoteUrl)` — the exact same warm-pool-aware service the home-screen `POST /api/repos/:url/claim-session` route uses. The child gets a workspace branched off freshly-fetched `origin/main`. When the parent's repo isn't "ready" — registered remote that flipped state, ad-hoc local repos, integration tests — the spawn falls back to a local clone (from the bare cache when a `remoteUrl` is set, else from the parent's workspace).
4. **Branch off `base`** — if `base` is provided, hard-reset the claimed workspace to that ref (`git reset --hard`). When `base` is omitted, the default is `origin/main` whenever the parent has a `remoteUrl` (claim path *and* fallback path both resolve `origin/HEAD` in the child's clone, fetching first when credentials are configured); only truly-local repos with no `remoteUrl` fall back to the parent's HEAD. The previous default — "parent's HEAD" — was changed because spawned children inherited the parent's committed-but-not-merged WIP, making their "Changes vs main" diff include work that was already on main. Honoring the user's expectation that a spawned session looks like any other new session was the goal.
