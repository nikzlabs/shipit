...
The user wants **two** independent reasoning controls, because the two places an
agent runs have different ownership:

| Control | Where | Scope | Governs | Source of truth |
|---|---|---|---|---|
| **A — Global default** | Settings window | Global, **per agent** | **Sub-agent** spawns (`shipit agent run --agent <id>` from inside another session) — these run server-side with no composer UI | `CredentialStore` map (server) |
...
They are **independent**: the global Settings value does *not* drive the main
session's turns, and the composer value does *not* drive sub-agents. This matches
the user's framing — the Settings control "only affects sub-agents." A sub-agent
...
1. **Store** — `CredentialStore` (`orchestrator/credential-store.ts`): add
   `agentReasoning?: Record<string, string>` + `getAgentReasoning(agentId)` /
   `setAgentReasoning(agentId, value | null)` (null clears → default), each calling `save()`.
   Persists to the global `/credentials/shipit-credentials.json`, alongside `autoCreatePr`, `enableSubAgents`, etc.
2. **Global settings surface** — add `agentReasoning` to the `GlobalSettings` type and to
   `getGlobalSettings()` / `saveGlobalSettings()` (`orchestrator/services/settings.ts`); extend
   `PUT /api/settings` (`api-routes-bootstrap.ts`) to accept a partial `agentReasoning` and merge
   per-agent. Returned in `/api/bootstrap`.
3. **Validation** (`orchestrator/validation.ts`): agentId must be registered and the value must be
   in that agent's `reasoning.options` (or null). Reject with `ServiceError(400, …)`.
4. **Sub-agent run-params** — thread `reasoningEffort` through `buildSubAgentRunParams`
   ...
   `getAgentReasoning(subAgentId)` so the invoked backend gets **its own** global setting.
5. **Settings UI** — add a per-agent reasoning row to the Settings window (one selector per
   registered agent that has a `reasoning` capability), reading/writing the `agentReasoning` map via
   the settings store → `PUT /api/settings`.
...
- **Control A:** `orchestrator/credential-store.ts`, `services/settings.ts`, `api-routes-bootstrap.ts`,
  `validation.ts`, `shared/sub-agent-run.ts`, `services/sub-agent.ts`, Settings UI component
