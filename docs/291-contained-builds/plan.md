...
## If the builder is ever put under a VM runtime

Recorded because it is a constraint on a *later* decision, not a change to this
one. `docs/292` evaluated per-session microVMs and recommended against them,
but named a narrower option: run the privileged builder — and only it — under a
VM-backed container runtime selected through `HostConfig.Runtime`, the same
mechanism `SESSION_RUNTIME=runsc` already uses for the agent container. A
privileged container is privileged inside the *guest* kernel, which is what a
root `RUN` step wants.

**No shape in this document survives that unchanged**, and the reason is the
first fact in *The tier stack is OUTPUT-only*. `filter/OUTPUT` matches packets
generated by a local socket in the namespace. A guest's traffic is not locally
generated: it arrives on a tap device as ingress. So a tier stack installed into
the host-side namespace polices nothing a VM emits, whatever the shape. Kata's
default networking makes this concrete and closes the obvious workaround —
TC-filter redirects `eth0` ingress to `tap0_kata` egress and back
(`kata-containers/docs/design/architecture/networking.md`), below netfilter, so
adding forward-chain rules does not help either. Its L3-forwarding model exists
precisely so host-side rules do apply, and is experimental and IPv4-only.

The consequence is that such a tier needs the egress policy expressed
**host-side, on the session's own bridge**, rather than installed into a
namespace by a sidecar that joins it.

**Do not pre-emptively design for that.** Moving this surface host-side means
giving up reuse of the Tier A/B/C stack for it — a second policy program that
must agree with `egress-allowlist.ts`, which is the outcome this document
argues against everywhere else, and which planning#383's history is the warning
about. Paying that to preserve optionality on a tier nobody has decided to
build is the wrong trade. What is worth doing now costs nothing: keep the
builder's snapshotter storage (`/var/lib/buildkit`) on a path that can become a
block or tmpfs volume, since a virtio-fs mount is not a valid overlay upperdir
and a VM runtime would need that path moved anyway.
...
and recorded its trigger; that stays deferred, `docs/292` re-checked it and
agreed, and this design does not reopen it. It should be re-read when planning#510 ships, because two things here sit
