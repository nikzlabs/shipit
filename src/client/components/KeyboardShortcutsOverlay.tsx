    ...
    <span className="flex items-center gap-1">
      {keys.map((key, i) => (
        <span key={i}>
          ...
          <kbd className="inline-block min-w-[1.5rem] text-center px-1.5 py-0.5 text-xs font-mono rounded bg-gray-700 border border-gray-600 text-gray-200">
            {key}
          </kbd>
        </span>
      ))}
...
export function KeyboardShortcutsOverlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="max-w-lg w-full mx-4 rounded-xl bg-gray-900 border border-gray-700 p-6 space-y-5">
        ...
        <p className="text-xs text-gray-600 text-center pt-2 border-t border-gray-800">
          Press <kbd className="px-1 py-0.5 text-xs font-mono rounded bg-gray-700 border border-gray-600 text-gray-300">?</kbd> to toggle this overlay
        </p>
      </div>
    </div>
  );
