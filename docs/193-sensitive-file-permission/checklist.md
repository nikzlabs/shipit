...
- [ ] Verify end-to-end in a real container session (manual): `.npmrc` edit surfaces the card; approve lets the write land; "remember" suppresses re-prompts
- [ ] Confirm the Codex deny enum against `codex app-server generate-json-schema` (allow path verified)
