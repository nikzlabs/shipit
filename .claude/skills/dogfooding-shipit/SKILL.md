...
`GITHUB_TOKEN` plus **any service credential you want to exercise** is the set. The `dev` service's `x-shipit-secrets` block in `docker-compose.yml` declares every `storageEnv` name the model catalogue knows (read it there for the current list), so a key set once out there appears in every dogfood session without a visit to inner Settings (docs/131 req 11).
