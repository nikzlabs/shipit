...
export function composeReviewMessage(filePath: string, opts: ReviewComposition): string {
  ...
  if (opts.mode === "role") {
    lines.push(
      ...
      "the consult card, which names the reviewer it picked. You do NOT record it and",
      "you call NO tool. Read the markdown from stdout and use it only to apply fixes",
      "and (optionally) re-review.",
      "",
