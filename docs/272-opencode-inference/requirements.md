...
8. **OpenCode Zen's Ox Alpha is offered**, on the pasted Zen key like any other
   ...
   so it overlaps none of ShipIt's existing model families — which is the test
   the 2026-08-17 subset answer set. The human named this model directly; the
   subset rule is unchanged for models nobody asked for.
...
- **2026-08-21 (Nik, chat)** — *Ox Alpha.* "Let's support ox alpha model from
  OpenCode." → req 8 added. Recorded because the row does NOT satisfy the
  subset answer above — Ox Alpha is a stealth model and overlaps no existing
  family — and an agent may not narrow a human's rule to fit its own row. What
