...
- [x] **Rebase driver left `.git` root-owned (SHI-144).** The user-driven rebase + auto-conflict-resolve flow (`services/rebase-driver.ts`) runs its git ops (fetch / rebase / `--continue` / stageAll / forcePush / abort) as the root orchestrator but dispatches resolution turns with `postTurn: "none"`, which elides the `postTurnCommit` `.git` handoff every normal turn gets — so the non-root agent's next in-container `git` failed on a root-owned `.git`. Fixed by calling `chownWorkspaceGitToSessionWorker(runner.sessionDir)` (a) before each agent resolution turn, (b) in a `finally` wrapping the whole `runRebaseFlow` (every exit path: clean / resolved / up-to-date / abort / throw), and (c) on the auto-path stale-rebase pre-flight abort + post-`Promise.race` (covers the timeout-teardown abort). Gated/no-op when the flag is unset. Tests in `rebase-driver.test.ts` spy the handoff across the up-to-date / clean / conflict / throw paths.
