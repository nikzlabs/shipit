...
## ⛔ Blocker — SHI-31 NOT done

- [ ] **Overlay dep store × non-root: agent can't `npm install` ([SHI-145](https://linear.app/shipit-ai/issue/SHI-145)).** Dev validation (flag on, `OVERLAY_DEP_STORE=1`) found the overlay `node_modules` volume (`shipit-<id>_overlay-<hash>` → `/workspace/node_modules`, docs/183) is **root-owned**, so the uid-1000 worker gets EACCES on `npm install` (top-level `/workspace` writes still work — only the overlay dep dir fails). The overlay path (`overlay-session.ts`) has **zero** worker-uid chown; the entrypoint's one-shot `/workspace` chown doesn't reliably cover the separately-mounted overlay (ownership is inconsistent across sessions). Overlay is **default-ON incl. prod** (`isOverlayEnabled`: empty `OVERLAY_DEP_STORE` → on), so this is **live in prod** and is likely the root-owned `node_modules` a prod agent already reported. Fix: hand the per-session overlay upper/work dirs + the shared base to the worker uid (gated on `sessionWorkerUid()`) — see SHI-145. **SHI-31 stays open until this lands** (the agent must be able to add deps in the default config).
