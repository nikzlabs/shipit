...
2. **Minimum age of 7 days** since npm publication — the window lets scanners and the registry's abuse pipeline catch a compromised release before it reaches our build. If you genuinely need a younger release (a security fix in a transitive, a CLI bump the user asked for by name), get explicit sign-off and record it as an `AGE_EXEMPTIONS` entry in `scripts/check-dependency-age.ts`: one exact `manifest + name + version`, the reason and who signed it, and an `expires` date set to the day that version turns 7 days old. It waives the age rule only — never the pin rule — and only for the version it names, so the next bump is gated normally. Merging past a red `check-deps` is the thing this replaces: it leaves no record of what was skipped or why.
