...
- Codex app-server may report the legacy 272K-class window for GPT-5.6 at turn completion. Because that late event otherwise overwrites the correct 1.05M `agent_init` fallback precisely between turns, `reconcileReportedContextWindow()` keeps the larger explicitly known GPT-5.6 window. Other model families continue to use backend telemetry unchanged.
