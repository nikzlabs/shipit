...
- `shipit agent run` — one-shot call to a DIFFERENT agent backend (Codex), whose answer you fold into your own work. Background it if it may be long; a killed call does not kill the run, and `shipit agent result` re-reads it. No sidebar session, no separate PR.
