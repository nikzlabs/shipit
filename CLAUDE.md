...
When a doc describes UI whose layout is load-bearing (filters, tables, responsive breakpoints), commit the prototype into the feature folder as a sibling of `plan.md` — `mockup.html`, `mockup.svg`, or a `mocks/` subdir for several. Prefer self-contained, static artifacts (one HTML file with inline CSS, or SVG) so they open with no build step and stay diffable; `.png` screenshots are an acceptable supplement but aren't diffable, so keep HTML/SVG as the source of truth. Link the mock from `plan.md` with a short "Visual reference" note so a reader lands on the picture — see `docs/173-issue-tracker-filters/mockup.html` and its "Visual reference" callout for the canonical example. The `present` tool's tab is ephemeral and never touches the repo; committed mocks are reviewable in PRs, render in the file tree, and survive across sessions.
