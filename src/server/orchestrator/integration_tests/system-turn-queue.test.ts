...
describe("Integration: a dispatched system turn behind a real turn (planning#256/planning#257)", () => {
  ...
  it("docs/295: a typed message is QUEUED while another send is in its pre-turn phase", async () => {
    // The pre-turn phase (the merged-session compaction, then the branch move)
    // runs inside a send that has already been accepted. It can spawn a CLI and
    // wait minutes on it, and a second message admitted there would race the
    // first for the one `_agent` slot — two agents, one working tree.
    const client = await TestClient.connect(port);
    await client.receive(); // preview_status
    ...
    const before = lastClaude;
    client.send({ type: "send_message", text: "typed mid-compaction" });
    await drainUntil(client, (m) => m.type === "message_queued");
    expect(runner.queueLength).toBe(1);
    expect(lastClaude).toBe(before); // no second agent was spawned

    // And it is not stranded: the phase releases the hold, and the send it was
    // holding for drains the queue at its own turn boundary.
    runner.preTurnHold = false;
    expect(releaseQueuedTurn(runner)).toBe(true);
    const resumed = await waitForClaude(() => lastClaude, before ?? undefined);
    expect(resumed.lastPrompt).toContain("typed mid-compaction");
    resumed.finish("resumed-session");

    client.close();
  });

  it("docs/295: a finished turn's WS drain does not start a turn beside a pre-turn phase", async () => {
    // The interactive twin of the dispatched drain guard. This drain runs from
    // the gap between `tryDrain` clearing `running` and the local commit
    // resolving, so it can land squarely inside another send's pre-turn phase —
    // and start a second turn against the same tree, racing for the same agent
    // slot. The phase's own turn drains the queue when it ends.
    const client = await TestClient.connect(port);
    await client.receive(); // preview_status
    ...
    const before = lastClaude;
    first.finish("first-session");
    // Settle time, not a state wait: `!runner.running` is true for an instant
    // BEFORE the drain runs, so waiting on it would pass whether the drain stood
    // down or spawned a turn a moment later — which is exactly how the first
    // version of this test passed with the guard removed.
    await new Promise((r) => setTimeout(r, 300));

    expect(lastClaude).toBe(before);   // the drain did not spawn
    expect(runner.queueLength).toBe(1); // …and did not lose the entry
    runner.preTurnHold = false;

    client.close();
  });

  it("docs/295 req 12: a `/compact` arriving mid-phase is QUEUED, not fired at the maintenance process", async () => {
    // The `/compact` branch returns unconditionally, so under the hold the
    // command was spent either way: with an empty slot it evaporated — no
    // compaction, nothing queued — and once the automatic compaction's proxy was
    // installed it was handed to THAT process, a nested request against a spawn
    // the user never made. Queuing is what makes their one compaction happen
    // once, after the phase, as its own turn.
    const client = await TestClient.connect(port);
    await client.receive(); // preview_status
    ...
    const runner = runnerFor(client.sessionId);
    await waitUntil(() => !runner.running, "first turn finished");
    // A resident process in the slot — the shape in which the command WOULD be
    // fired at whatever is installed there.
    runner.setAgent(first as never);
    ...
    expect(first.compactCalled).toBe(false);
    expect(runner.queueLength).toBe(1);
    runner.preTurnHold = false;
  ...
  it("docs/295: an AskUserQuestion answer is QUEUED during a pre-turn phase too", async () => {
    // `answer_question` does not go through `dispatch`: it sets `running = true`
    // and calls `runAgentWithMessage` itself, so it needs the hold asked
    // explicitly — the same reason docs/288 had to add it there for the merge.
    const client = await TestClient.connect(port);
    await client.receive(); // preview_status
