...
**Close pull request** appears in two places so it's both discoverable in the common case and reachable when merging isn't possible. For the regular (mergeable) case it's an item inside the merge button's dropdown, separated from the merge methods and styled in `--color-error` — the most discoverable spot, right where the user already is. But the merge button only renders when the PR is mergeable (CI green, no conflicts, review satisfied), so for every state where merging isn't possible — most importantly **merge conflicts**, but also failing CI, a pending required review, or auto-merge armed — close also lives in the card's existing overflow menu (the always-present `⋮` `OverflowMenu` that already hosts Sync / Copy branch name / Download chat). On the inline card it's appended to that menu whenever the PR is `open`; in the detail panel (`PrStatusSection`), which has no persistent overflow menu, the same `OverflowMenu` is rendered in the action row only when the merge button is hidden (`!showMergeButton`). Reusing the existing menu — rather than adding a second standalone kebab next to the merge button — keeps the card free of duplicate `⋮` triggers. Folding close *only* into the merge dropdown was the original bug: a conflicted PR had no way to be closed from the card at all.
