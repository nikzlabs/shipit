...
**A message that is BOTH broadcast live and rehydrated from history needs an identity that survives both.** The live copy *appends*; the history load *replaces* the whole transcript. They race on every attach, reload and switch, and whichever lands second decides whether the user sees the message zero times or twice — the two failures look nothing alike, so one is always still open after you fix the other. Matching on text is not an identity: two "continue" sends are genuinely distinct messages, so a text comparison either collapses them or duplicates them. Give the message a stable id, **persist it on the row** (`messages.client_request_id`, carried by the `system_user_message` echo), reconcile on that id anywhere in the transcript rather than at the tail, and **queue the live message behind `historyLoaded`** in `useMessageHandler` alongside `agent_event` / `turn_snapshot`. Then any arrival order converges.
