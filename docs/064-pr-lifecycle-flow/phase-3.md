...
3. Excess merged sessions beyond `MAX_MERGED_SESSIONS_PER_REPO = 3` (per repository) are archived — `archiveSession` disposes the runner, drops named volumes, and removes the workspace directory. The "keep the 3 most recent" ranking is by **most-recent activity** — `max(merged_at, last_used_at)` parsed in JS (the two columns use different on-disk formats, so a lexical SQL `MAX()` would be wrong) — **not merge time alone**. This protects a session merged long ago that the user has returned to (e.g. to open a follow-up PR): its bumped `last_used_at` keeps it out of the archive slice. `last_used_at` is refreshed at turn *start* (`runAgentWithMessage`) as well as turn end (`agent-listeners` on `agent_result`), so resuming work registers immediately. As a hard backstop, a session whose agent is **currently running** (`runnerRegistry.get(id)?.running`) is never auto-archived — archiving force-disposes the runner and would kill a live turn and delete the workspace mid-work.
