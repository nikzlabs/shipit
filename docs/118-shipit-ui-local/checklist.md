...
- [ ] Confirm `x-shipit-secrets` resolves correctly: `secret-resolver.ts:writeServiceEnvFilesToRoot` writes `.env.dev` to `<serviceEnvDir>/<sessionId>/` — outside the workspace volume, since planning#292 deleted the in-workspace `writePerServiceEnvFiles` path — and `compose-generator.ts` adds an absolute `env_file:` to the resolved compose service. Verify with one end-to-end test.
- [x] **Secrets-leak mitigation.** ~~The default secrets path lands `.shipit/.env.dev` in `${workspaceDir}` — i.e. the ShipIt repo source tree.~~ **No longer true since planning#292**: the default path is `<serviceEnvDir>/<sessionId>/.env.dev`, outside the workspace, and the in-workspace writer is deleted, so there is no leak into the source tree to mitigate. The `SHIPIT_SECRETS_INTERNAL_DIR` note below still applies as the stronger Docker-secrets option:
...
- [ ] **The `shipit` MCP bridge stays absent in local mode (planning#305).** Every tool on it POSTs to the worker's `/agent-ops/*` — a host local mode does not have — so it is configured as `null` rather than advertised-and-broken (which would also point Claude's `--permission-prompt-tool` at an unreachable broker). So no `present` / `voice_note` / `propose_actions` / `report_shipit_bug` in dogfood turns. **The `gh` shim is no longer part of this (docs/251)** — the shims were assumed to need the same host as the bridge, but `gh` is a pure relay while the bridge's tools are worker-served, so `gh` now works in the dogfood via `orchestrator/local-agent-ops.ts`. planning#305 keeps the bridge and the `shipit` shim.
...
- [x] Closed the reserved-route seam: `clearAgentHomeCredentialLinks()` removes stale account links when the turn routes to a reserved env route, so the home never carries one route's credentials while the turn runs on another (docs/150-multiple-provider-subscriptions req 12 by construction, not by the CLI's env-beats-disk luck). 5 new tests.
...
- [x] The operational half of the env-var story, confirmed the hard way: with `ANTHROPIC_AUTH_TOKEN` set on the `dev` service, turns failed with `Agent CLI requires authentication` — a symptom identical to planning#284 and *not* it. Unsetting it in outer Settings → Secrets is what let the first turn through. `scrubEnvAuthForScopedHome` means that is no longer required, but the incident is why the docs still say to leave it unset.
  (Inner sessions having no MCP servers — first recorded here from the missing-browser-tool symptom — is the same gap; consolidated into the planning#300 entry under **Follow-up** above, now fixed, so there is one record, not two.)
