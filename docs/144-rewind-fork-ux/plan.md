...
**Refinement (planning#184, planning#186) — fork is exempt from the running-turn gate.** B4's original "gate *all* handlers on `runner.running`" was too broad for `fork`. A fork spins off a **new** session from an already-committed SHA and never mutates the current one (its only parent-side effect is the breadcrumb append), so a running turn is no reason to block it — and the blanket gate stranded users trying to fork a session whose turn was hung. The server gate now reads `runner?.running && action !== "fork"`; in-place rewind (chat/code/both) stays gated. The client mirrors this: `RewindPoint` takes a `turnRunning` prop (instead of a blanket `disabled`) and, while a turn runs, presents the gap menu as **fork-only** rather than disabling the whole control. Two consequences follow: (1) the fork base is resolved to a concrete committed SHA up front (HEAD for a current-state fork, the most recent commit before the gap otherwise) so a concurrent end-of-turn auto-commit can't shift it under the `git clone --local`; and (2) `clearQueuedMessages` is skipped for fork — fork leaves the parent intact, so the parent's queued messages must survive. Separately (planning#186), a session with **no auto-commits** has no per-message commit, so `findCommitBeforeGap` returns null — fork now falls back to HEAD instead of erroring with "No code state available to fork." Key file: `rollback-handlers.ts` (`handleRewindAtGap`, `action === "fork"` branch). Tests: `rewind-fork.test.ts` ("forks a session with no code commits…", "allows fork while a turn is running…"), `RewindPoint.test.tsx`.
...
> **Revision (TRACKER-26) — click-handle model, no hover-reveal.** The original spec below (full-width hairline + a button that fades in on hover, plus an always-visible pill on the gap-after-last) shipped, but proved too heavy in practice for a rarely-used action: the 24px hover band reserved a large gap between every turn, and because the *whole* band was the hover target, the Rewind button appeared whenever the cursor merely crossed a gap. The shipped control is now:
...
> Superseded by the TRACKER-26 revision above: intermediate and gap-after-last now share a fixed `h-2.5` (~10px) row with a click-opened pip (40% → ~90% on hover/focus), no full-width hairline, no fade-in button. The table below reflects the original hover-reveal design.
