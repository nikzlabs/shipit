...
- **Webhook URL:** the user's own tunnel URL + `/api/webhooks/linear`, configured on the user's own Linear app registration.
- **Webhook events:** `Agent session events` — gives us `AgentSessionEvent.created` and `AgentSessionEvent.prompted`.
- **Webhook secret:** generated by the user when creating the Linear app, pasted into ShipIt settings and stored in `CredentialStore`.

The user then installs *their own* Linear app onto their own workspace (workspace admin required by Linear). After install they paste the client ID, client secret, and webhook secret into ShipIt settings.
