...
- **Local mode intentionally diverges from the non-root container hardening (docs/150).** Session-worker *containers* drop to the unprivileged `shipit` user (UID 1000, home `/home/shipit`) via a `gosu` entrypoint. Local mode has no such container: the inner orchestrator runs as **root** in the `dev` service and hosts the agent processes in-process (`buildLocalAgentFactory` → `claude-adapter`/`codex-adapter`). It therefore keeps `HOME=/root` and **pins `AGENT_HOME=/root`** (in `docker/Dockerfile.dogfood` and the `dev` compose service) so `agentHome()` — whose default is `/home/shipit` — resolves credentials under `/root` where the dogfood image actually wrote them. Do **not** create a `shipit` user in `Dockerfile.dogfood` or set `SHIPIT_SESSION_WORKER_UID` on the inner orch; the orchestrator-side chown helpers (docs/150 §7) are no-ops without it, which is correct for local mode. The shared `node_modules` mount caveat (writes from the root `dev` service vs. the non-root outer agent) is covered in docs/150 §9.
