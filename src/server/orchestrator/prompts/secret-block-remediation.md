...
2. If it is a real credential, remove it. Replace it with an environment variable read (`process.env.X` or the language equivalent) and, if the value is genuinely needed at runtime, tell the user to add it under Project Settings → Secrets. Never invent a placeholder that still looks like a token.
