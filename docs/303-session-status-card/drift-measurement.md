...
### 2. A text-only turn never updates the card

Not once in 46 turns. These are not the exempt case: 45 of the 46 ended with a
statement, not a question. They are the turns that answer a question in prose
and stop — the shape the user described as "Okay, everything is good".

This class also gets no nudge. Why the nudge does not fire here is **not
established**. The candidate gates in `shouldNudgeForStatusCard` are
`wasInterrupted` and `successorPending`; neither was confirmed against a
reproduction, and the database does not record which gate applied. Whoever
implements a fix must establish this from the code before trusting it.
...
turns with no call), and only 81 nudges — 48%.
...
the same definitions. The three numbers that matter are the work-turn call rate,
the text-only call rate, and the share of calls that carry neither `needsYou`
nor `actions`.
