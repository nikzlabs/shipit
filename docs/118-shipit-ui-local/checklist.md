...
- [ ] Local-mode agents receive no MCP configuration. `invokeAgentMcpWriter()` is called only by `AgentController`, and `McpConfigController` is constructed only by `session-worker.ts`; `RUNTIME_MODE=local` has no session worker and its plain `SessionRunner` has no equivalent MCP-writing path. Consequently neither backend gets the Playwright/browser MCP server, the ShipIt bridge tools, or user MCP servers in dogfood turns. Wire MCP configuration into the local runner path and cover both Claude and Codex; do not infer coverage from the containerized worker path.
