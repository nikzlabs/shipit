...
| `src/server/orchestrator/local-agent-home.ts` (docs/150-multiple-provider-subscriptions req 19) | **The per-turn mechanism.** `resolveLocalAgentHome()` answers, at spawn time, which account root a local session's CLI should use as `HOME`; `undefined` (reserved route, or a cross-provider sub-agent whose provider has no selectable account) falls back to `agentHome()`. Bound to each local runner's `createAgent` in `app-lifecycle.ts`. |
