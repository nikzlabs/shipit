...
# PR card changed-docs strip — keep it current
...
- **Recompute every post-turn commit.** In `emitPrLifecycleAfterCommit`, the
  existing-PR branch no longer bare-returns: it re-derives the notable list from
  the current branch and emits a new lightweight **`pr_notable_files`** WS
  message (`{ sessionId, cardId, notableFiles }`). The client patches *only*
  `notableFiles` on the live card (`pr-store.setNotableFiles`), leaving the
  poller-owned fields (phase/pr/checks) untouched, so the strip tracks the
  branch turn-by-turn without fighting the poller. The list is authoritative —
  an empty array clears the strip.

- **Merge-base diff.** `notableFilesForBranch` now resolves the base ref, finds
  `merge-base(base, HEAD)`, and diffs `mergeBase..HEAD` — the same "what this
  branch changed" set the diff stat and Docs panel use. The strip and the diff
  stat now line up (which docs/205 intended but didn't achieve), and
  base-advanced files no longer appear.
