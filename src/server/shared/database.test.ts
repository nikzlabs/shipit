...
describe("docs/299 — install-level usage rows (real migration)", () => {
  let file: string;
  let dir: string;

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "shipit-299-usage-"));
    file = join(dir, "test.db");
  });
  ...
  const turnSeed = (id: number): TurnSeed => ({
    ...
    rate_input: id + 0.28,
    rate_output: id + 0.42,
    rate_cache_read: id + 0.028,
    rate_cache_write: id,
    credential_route_id: `route-${id}`,
    cumulative_tokens_repaired: 1,
  });

  /** The shipped shape immediately before the migration: NOT NULL, no classification. */
  function rewindAndSeed(turns: TurnSeed[]): void {
    const m = new DatabaseManager(file);
    m.db.exec(`
      DROP TABLE usage_turns;
    ...
    for (const t of turns) insert.run(...PRE_MIGRATION_COLUMNS.map((c) => t[c]));
    // The session id it cannot yet hold, which is what this migration is for.
    expect(() => insert.run(...PRE_MIGRATION_COLUMNS.map((c) => (c === "session_id" ? null : turnSeed(9)[c])))).toThrow(
      /NOT NULL/i,
    );
    m.db.pragma(`user_version = ${INSTALL_LEVEL_USAGE_MIGRATION}`);
    m.close();
  }
  ...
  it("accepts a row belonging to no session, which the populated database could not hold", () => {
    rewindAndSeed([turnSeed(1)]);

    const m = new DatabaseManager(file);
    m.db
      .prepare("INSERT INTO usage_turns (session_id, cost_usd, duration_ms) VALUES (NULL, 0.5, 400)")
      .run();
    const row = m.db.prepare("SELECT * FROM usage_turns WHERE session_id IS NULL").get() as Record<
      string,
      unknown
    >;
  ...
  it("leaves every pre-existing row unclassified, since sub_agent_id cannot identify background work", () => {
    rewindAndSeed([turnSeed(1)]);
  ...
  it("keeps the all-or-none attribution CHECK across the rebuild", () => {
    rewindAndSeed([turnSeed(1)]);
    ...
    expect(() =>
      m.db
        .prepare(
          "INSERT INTO usage_turns (session_id, cost_usd, duration_ms, service_id) VALUES ('s', 1, 1, 'deepseek')",
        )
        .run(),
  ...
  it("is a no-op when re-run, so an earlier migration's test rewind cannot drop a classification", () => {
    rewindAndSeed([turnSeed(1)]);
    const m = new DatabaseManager(file);
    m.db.prepare("UPDATE usage_turns SET background_work = 1").run();
    m.db.pragma(`user_version = ${INSTALL_LEVEL_USAGE_MIGRATION}`);
    m.close();
