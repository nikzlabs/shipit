...
**User message queued during the resolve turn.** The `"idle"` predicate is `!_isRunning && _messageQueue.length === 0` (`session-runner.ts:574`). If a user types a message while the auto-resolve's agent turn is mid-flight, the message lands in the runner's queue; when the resolve turn finishes, `onAgentFinished()` sees the queue is non-empty and does NOT emit `"idle"`. Unlike the WS user-turn path (which calls `drainNextQueuedMessage` from inside `agent.on("done")`), `runRebaseResolutionTurn`'s `done` handler (`rebase-driver.ts:296-307`) does NOT drive a drain — it only resets `running` / `setAgent(null)` / fires `session_agent_finished` / calls `onAgentFinished()`. So the queued user message would sit stranded until the next WS `send_message` arrives and the dispatch path picks it up. To avoid this, `runRebaseResolutionTurn`'s `done` handler must also call `drainNextQueuedMessage` (or the rebase-driver must accept a drain callback through `RebaseDriverDeps`). The fix touches doc 094's rebase driver, not this manager — call it out as a doc-094-side change required by doc 146. Net once fixed: a user message during an auto-resolve turn drains automatically after the resolve completes, eventually emits `"idle"` when *its* turn finishes, and the `deferred` re-evaluation kicks in — the correct behavior (the user clearly has more work for the agent, so don't queue another auto-resolve ahead of them).
