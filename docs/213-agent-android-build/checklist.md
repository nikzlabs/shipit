...
- [ ] Phase 4: agent `adb` debug/drive loop (logcat + tap/screenshot/snapshot triad, optional Maestro) + interactive preview via `x-shipit-preview`
  ...
  - [x] **Dogfood target retargeted:** the `android/` WebView wrapper was deleted (superseded by the PWA, docs/222 / planning#210), along with `.github/workflows/android.yml`. The preview stack now builds **`android-snapshot-test/`**, which gained a launchable `MainActivity` (rendering the same `GreetingCard` as the snapshot golden) so it can be installed + launched on the emulator. Strictly better than the wrapper, which `layoutlib` could never render.
  - [x] **Service-start failure fixed:** the emulator died on its entrypoint with `sh: /home/androidusr/…/run.sh: Permission denied` — ShipIt forces the session-worker UID onto every compose service (docs/150 §7) and the budtmo image runs as its own baked-in `androidusr`. Fixed by declaring `user: androidusr` in the recipe (an explicit `user:` is honored verbatim), documented as a general rule in `compose.md`, pinned by a named-user regression test. The `android` builder keeps the worker UID (it writes to the shared workspace) but gained a writable `HOME`/`GRADLE_USER_HOME`.
  - [ ] **Host verification (operator, needs KVM + image build):** start the `emulator` preview on a KVM host; confirm port 6080 serves noVNC and renders in the preview pane, the `android` builder builds + installs the APK, and editing `android-snapshot-test/app/src` triggers a rebuild+relaunch. (Also confirm Compose service containers have egress for Gradle dep resolution.)
