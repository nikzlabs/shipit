...
2. **`MainActivity`** — the WebView host. On `onCreate`, reads the saved URL. If absent, launches `SettingsActivity` and finishes. Otherwise loads the URL into a full-bleed `WebView`. Configures: JS, DOM storage, third-party cookies (needed for the websocket session), `WebChromeClient.onShowFileChooser` for attachment uploads, hardware back button = `webview.goBack()` with fallback to default. There is **no toolbar** — the WebView is truly full-bleed. The two actions that used to live in a toolbar overflow are now chrome-free affordances: a small translucent **settings cog floating at top-center** (`@drawable/ic_settings`, ~32dp, `alpha 0.45`) re-launches `SettingsActivity`, and **pull-to-refresh** (an `androidx.swiperefreshlayout` wrapping the WebView) replaces the explicit Reload. Top-center placement deliberately avoids the corners, where ShipIt's own web UI keeps its controls.

> **On-device caveat:** ShipIt's web UI uses internal scroll containers, so the WebView document itself may sit at `scrollY == 0`, which can make `SwipeRefreshLayout` over-trigger on downward swipes. If pull-to-refresh conflicts with the chat scroll on real devices, the fallback is to drop `SwipeRefreshLayout` and fold Reload into the cog (e.g. long-press = reload, tap = settings). Flagged for the manual smoke test.
