...
**Close pull request** lives in its own standalone overflow menu (`ClosePrButton`, a kebab `⋮` trigger), **not** in the merge dropdown. This is deliberate: the merge button only renders when the PR is actually mergeable (CI green, no conflicts, review satisfied), but closing a PR must stay available in exactly the states where merging isn't — most importantly when the branch has merge conflicts. Folding close into the merge dropdown meant a conflicted PR had no way to be closed from the card at all. So `ClosePrButton` renders whenever the PR is in the `open` phase, regardless of merge-ability, on both the inline card (`OpenPhase`) and the detail panel (`PrStatusSection`). The single close item is styled in `--color-error` as the one destructive action and uses a two-step confirm (first click arms it → "Click again to confirm") so a misclick can't close a PR; the second click calls `closePr` in `pr-store`, which `POST`s to the existing `/api/sessions/:id/pr/:number/close` route and optimistically flips the card to the `closed` phase. This keeps closing a PR inside ShipIt rather than bouncing the user to GitHub. Closed-but-not-merged PRs get a distinct red badge (`PrStateBadge`) using GitHub's `git-pull-request-closed` Octicon — `GitPullRequestClosedIcon`, a one-off custom SVG since Phosphor has no closed-PR glyph — so a closed PR no longer looks like a plain gray branch.
