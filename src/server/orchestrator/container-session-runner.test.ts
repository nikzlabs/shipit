...
describe("ContainerSessionRunner — the reconciler stands down during a pre-turn phase (docs/295)", () => {
  /** Reach the reconcile tick and the state it reads. */
  function recon(runner: ContainerSessionRunner): {
    runReconcileCheck(): Promise<void>;
    _isRunning: boolean;
    _viewerCount: number;
    _reconcileDivergenceCount: number;
  } {
    return runner as unknown as {
      runReconcileCheck(): Promise<void>;
      _isRunning: boolean;
  ...
  it("verifyRunningState itself stands down, for the callers that skip the tick", async () => {
    // `services/child-sessions.ts` calls `verifyRunningState()` directly from
    // `shipit session wait`, so a guard living only in the periodic reconciler
    // leaves the concurrent probe uncovered.
    //
    // Driven against a REAL worker answering `running: false` — the divergence
    // the net exists for. An unreachable worker would return the same `true`
    // from its own catch, which is what makes the cheap version of this test
    // pass with the guard removed.
    vi.useRealTimers();
    const server = http.createServer((req, res) => {
      res.setHeader("content-type", "application/json");
      res.end(JSON.stringify(req.url === "/agent/status" ? { running: false } : {}));
    });
    await new Promise<void>((resolve) => server.listen(0, "127.0.0.1", resolve));
    const addr = server.address();
    if (typeof addr === "string" || !addr) throw new Error("no server address");
    try {
      const runner = makeRunner();
      runner.setWorkerUrl(`http://127.0.0.1:${addr.port}`);
      const r = recon(runner);
      r._isRunning = true;
      runner.preTurnHold = true;
      const abandoned: unknown[] = [];
      runner.on("turn_abandoned", () => abandoned.push("abandoned"));

      expect(await runner.verifyRunningState()).toBe(true);
      // Without the guard: the slot and the delivery are cleared, the session is
      // told its agent went out of sync, and the turn is abandoned — all while
      // the pre-turn phase is still assembling the user's turn.
      expect(abandoned).toEqual([]);
      expect(r._isRunning).toBe(true);
    } finally {
      await new Promise<void>((resolve) => server.close(() => resolve()));
    }
  });

  it("resets the divergence count instead of asking the worker", async () => {
    // A turn's pre-turn phase publishes `running` and then legitimately has no
    // agent in the worker for as long as the merge probe, the eligibility check
    // and the compaction's env-prep take — which can exceed the two ticks this
    // net counts. Left counting, it would reset a turn that is merely still
    // starting and take the compaction's agent slot with it.
    const runner = makeRunner();
    const r = recon(runner);
    r._isRunning = true;
    r._viewerCount = 1;
    r._reconcileDivergenceCount = 1;
    runner.preTurnHold = true;

    await r.runReconcileCheck();

    // Zero, not one: the tick STOOD DOWN rather than falling through to the
    // worker call (whose own unreachable-worker path returns with the count
    // untouched, which is what this distinguishes it from).
    expect(r._reconcileDivergenceCount).toBe(0);
  });
});
