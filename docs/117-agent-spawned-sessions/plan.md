...
- **(Agent/model selection.)** `--agent`/`--model` are validated and
  ...
  rather than inheriting the parent's Claude. An unknown `--agent`, a
  `--model` whose owning backend conflicts with an explicit `--agent`, or a
  `--model` the resolved harness's catalogue does not list, is rejected with
  an actionable message — the once-accepted unlisted-id passthrough was
  removed by planning#304, which validates the *resolved* (agent, model)
  pair rather than only the explicitly-passed flags. `shipit session view` now also
...
3. **Get the child's workspace.** Spawn is a thin wrapper around `claimSessionService.claim(parent.remoteUrl, { skipReuse: true, excludeSessionIds: [parentId] })` — the exact same warm-pool-aware service the home-screen `POST /api/repos/:url/claim-session` route uses. The child gets a workspace branched off freshly-fetched `origin/main`. There is no local-clone fallback: keeping the two code paths unified means the child looks like a regular new session by construction, and any change to claim semantics flows through both surfaces.

   **`skipReuse: true` is load-bearing — it keeps a spawn from stealing a user's draft.** The claim service's *reuse* path (`findUngraduatedWarm`) recycles abandoned `warm = 1` sessions so the home-screen quick-capture flow (docs/145) doesn't leak warm slots. But a `/{repo}/new` page the user is actively typing in has *also* claimed a warm session, and it stays `warm = 1` until the first message graduates it. `findUngraduatedWarm` scans *every* `warm = 1` session for the repo and can't tell "abandoned draft" from "browser attached right now" — so a background spawn for the same repo used to alias the child onto the live draft, graduate it, and dispatch the child's first prompt into the very session the user was viewing. The symptom: a message appearing from nowhere while the user typed their first prompt. `skipReuse` disables the reuse path *for spawns only* (interactive claims leave it unset, so recycling your own just-abandoned draft still works); spawns take the pre-warmed pool — whose `warmSessionId` pointer is always cleared the instant a browser claims it — or a slow-clone, neither of which can collide with an interactive draft. `excludeSessionIds: [parentId]` is the narrower companion guard against a spawn aliasing onto the *parent* itself. Regression: `integration_tests/agent-spawned-session.test.ts` → "never recycles a user's ungraduated /{repo}/new draft".
4. **Stay on `origin/main`.** Generic fan-out children always stay on the claim's freshly-fetched `origin/main` — there is no agent-facing `--base`. The earlier "parent's HEAD" default was dropped because children inherited the parent's committed-but-not-merged WIP, and the `--base` override that replaced it was then removed too: an agent passing a stale ref (e.g. the parent's pre-merge branch tip) silently pinned the child to a commit that *missed* a just-merged parent change — most visibly, a child spawned to implement a design doc the parent had already merged couldn't see the doc. With the override gone, the child looks like any other new session by construction. The one remaining `base` is **internal**: the Ops `--shipit-source` path hard-resets the claimed workspace to the exact inspected build commit (`spawnChildSession`'s `opts.base`, set from `resolveShipitFixTarget().ref` — never from agent input). See `services/child-sessions.ts` and `integration_tests/ops-fix-spawn.test.ts`.
