...
- **Get the other backend's review before handing work over** — Claude-authored work reviewed by Codex and vice versa, via `shipit agent run --agent <other> --prompt-file -`. A same-model reviewer shares your blind spots. Use that, **not a subagent**: the Claude CLI's built-in "don't call the AgentTool unless asked" is baked into its binary and can't be turned off here, but a brokered run isn't a subagent, so it doesn't apply. You judge when work warrants it — a PR, a substantive change, any result you're about to call done — and it's advisory, so it never gates the PR. Background it (`run_in_background: true`); a real review outlives the 10-minute foreground cap, and `shipit agent result` recovers a dropped one. Give the reviewer `git diff main...HEAD` to run itself, not a paste. If the other backend isn't signed in, skip and say so.
