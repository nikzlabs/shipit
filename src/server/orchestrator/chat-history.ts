...
export class ChatHistoryManager {
  ...
  findRepoSessionProposalCard(sessionId: string, cardId: string): RepoSessionProposalCard | null {
    const rows = this.stmtLoadAll.all(sessionId) as MessageRow[];
    for (const row of rows) {
      if (!row.repo_session_proposal) continue;
      const card = JSON.parse(row.repo_session_proposal) as RepoSessionProposalCard;
      if (card.cardId === cardId) return card;
    }
    return null;
  }
