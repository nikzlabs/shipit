...
22. Delivery is surface-aware. While the ShipIt app is visible — desktop or mobile — new items surface through the existing visual attention surfaces, and the coordinator does not start speaking on its own.
23. An away mode exists. With the app closed or the phone locked, the user can mark themselves available; the coordinator then speaks arriving items immediately, one at a time, through a native phone channel. The user never needs to open the UI to know what is going on.
24. In away mode the user replies by voice through the phone's native assistant controls, and the reply reaches the coordinator without opening the UI.
