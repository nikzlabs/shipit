...
- [x] Req 34 (planning#589): a turn the user steered into is not nudged — `steeredMessages` joins `running` and the queue as a pending successor, so the user's message is never preempted by a system turn that retires the process holding it.
