...
- **`deployment/hetzner/deploy.sh` owns build-time prunes.** After each `docker compose build` it runs `docker image prune -af --filter "until=168h"` and `docker builder prune -f --filter "until=72h"`. BuildKit cache only grows because of builds, so this is where the prune is causal — duplicating it on a timer would just do redundant work. `--no-cache` is no longer the default (it was eating ~80 GB on prod); set `FORCE_REBUILD=1` for a clean rebuild. `NPM_GLOBALS_REBUILD=$(date +%s)` is passed every deploy so the Claude/Codex CLI install layer always picks up the latest published versions.
