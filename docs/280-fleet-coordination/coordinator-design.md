...
- Context: **epoched** *(amended 2026-08-23, req 20 — recommendation pending user ratification)*. The load-bearing distinction: **the ShipIt transcript and the agent's context window are different objects.** The transcript is one permanent, continuous conversation (req 14) — the user's scrollback never resets. The agent's *context* starts fresh per epoch: on the first engagement after a long idle gap (default: ≥6 h or a new day), the coordinator gets a clean conversation seeded from the memory repository (yesterday's short handoff note + preferences), presence, and the top of the queue. Old context is simply abandoned — nothing to compact, nothing lost, because fleet state was never in it and durable memory is in the repo. Harness-native compaction remains the intra-epoch fallback for one very heavy day. No custom compactor. At each epoch close the coordinator writes the handoff note — which is also the natural nursery for req 12's "patterns that prove stable."
...
- UI for a months-long transcript (req 20, constrained by req 21) — four mechanisms, all presentation-layer; the persisted record is never touched:
  1. **Windowed hydration.** `loadSessionHistory` gains a cursor: the client fetches only the recent window (latest epoch + tail), and older pages on demand. Today the client loads and renders the *entire* history on switch — fine for a weeks-long work session, unbounded for a permanent one. The store's `messages` array becomes a window; live WS appends to the tail exactly as now.
  2. **Bounded render window with collapsible day sections.** Loaded-but-old epochs mount as collapsed section headers (date + a one-line gist) that expand on demand; pages far above the viewport unmount again. The DOM then holds roughly one day of conversation regardless of age. **No per-row virtualization in v1** — variable-height bubbles and streaming cards are the classic hard case for it; escalate only if a measured single epoch is too heavy (req 12: harden what proves needed).
  3. **Chat-correct scroll.** Open lands on current state (queue rail + latest exchanges); prepending an older page preserves the visual position (record scrollHeight around the prepend); returning to the session restores where you were. Date separators stay the only visible seam (req 21).
  4. **Server-side history search.** With most rows unmounted, find-in-conversation queries the persisted history (the UI twin of the coordinator's self-history tool) and deep-loads the matching page.

  Scale check: a heavy year ≈ 30–50k message rows — trivial for SQLite; what breaks without this is the full-history JSON fetch and a six-figure DOM node count, and both are presentation problems. Mobile uses the same mechanism. (Benefits every long-lived session, but the coordinator makes it mandatory.)
...
1. **Context epochs + memory storage (reqs 17, 20 — now coupled):** the epoch model (fresh agent context per day over one permanent transcript) depends on a durable handoff location, which is exactly what the **git memory repository** provides (versioned, user-inspectable and editable from anywhere, history shows how behavior evolved, backup is a remote — GitHub optional; reuses auto-commit/push end to end; subsumes the preferences-durability question). Alternative: built-in storage with its own backup and viewer machinery, and a separate epoch-handoff mechanism. Recommendation: ratify both together — epochs + git memory repo.
