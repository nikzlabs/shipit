...
- **Desktop vs mobile indent + collapse** in one reflowing row: the title cell carries the disclosure caret + depth-indent (`@sm` only, so table columns stay aligned); the row's left padding carries the whole-card indent below `@sm`; and a `@sm:hidden` **"N nested issues"** button is the card layout's collapse control. The panel measures its `@container` width (`useNarrowContainer`, 384px) and renders the **narrow** section set (parents collapsed by default) or the **wide** set (expanded by default) accordingly — so the visible rows match the affordance shown. `collapsePredicate(overrides, narrow)` is the single resolver: explicit override wins, else `narrow` is the default.
