...
2. **`MainActivity`** — the WebView host. On `onCreate`, reads the saved URL. If absent, launches `SettingsActivity` and finishes. Otherwise loads the URL into a full-bleed `WebView`. Configures: JS, DOM storage, third-party cookies (needed for the websocket session), `WebChromeClient.onShowFileChooser` for attachment uploads, hardware back button = `webview.goBack()` with fallback to default. There is **no toolbar** — the WebView is truly full-bleed. The two actions that used to live in a toolbar overflow are now chrome-free affordances: a small translucent **settings cog floating at top-center** (`@drawable/ic_settings`, ~32dp, `alpha 0.45`) where **tap → Settings** and **long-press → reload**. Top-center placement deliberately avoids the corners, where ShipIt's own web UI keeps its controls.

> **Pull-to-refresh removed (the anticipated caveat landed):** ShipIt's web UI scrolls inside internal containers, so the WebView document sits at `scrollY == 0`. `SwipeRefreshLayout.canChildScrollUp()` only inspects the WebView's own scroll, so it reported "at top" for every downward swipe and stole the gesture — making chat history unscrollable on real devices. As the original belt-and-suspenders plan called for, we dropped `SwipeRefreshLayout` (and its dependency) entirely; **long-press → reload** on the cog is the Reload action. Reinstating pull-to-refresh would require a JS bridge that reports the active inner container's scroll position back to a synchronous `canChildScrollUp()` override — deferred as not worth the fragility.
