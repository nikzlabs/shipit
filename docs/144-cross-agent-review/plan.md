...
## Proposal — ShipIt renders what it brokers (open; pre-decision)

> **Status: proposal, not yet decided.** This section revisits §6 and §7 above
> and the central mechanism of `docs/203` (the parent calling `submit_review`).
> Nothing here is implemented; it is written to be reacted to before any code
> moves. The v0 behavior described in the rest of this doc and in `docs/203` is
> unchanged until/unless this is accepted.

### The principle

Draw the surfacing boundary at the **brokering** boundary:

- **Within one agent**, it is the agent's job to showcase a review (or any
  sub-task result) however it likes — prose, a markdown findings list, inline.
  There is no model boundary for the user to be told about, so ShipIt does not
  need a first-class artifact here.
- **Across agents**, ShipIt is the broker: it spawns the subprocess and holds
  the result. The boundary is real and only ShipIt can make it legible
  ("this opinion came from Codex, not Claude"). So **ShipIt** owns surfacing the
  cross-agent output — not the primary agent.

One line: **ShipIt renders what it brokers; the agent renders what is internal
to it.**

### Why the current design is inverted

`docs/203` routes *both* modes through the same parent-invoked `submit_review`
tool (its §3 + "submit_review" sections: the reviewer returns markdown, the
**parent** records the card, identically for a `Task` subagent and a
cross-agent spawn). That choice — "one uniform card path" — has two costs that
fall exactly where this principle says they shouldn't:

1. **The within-agent case gets a formal card it doesn't need.** A review that
   never left the agent is given a bordered `aiReview` artifact via a dedicated
...
2. **The cross-agent case — the one place ShipIt should be authoritative — is
   ...
   primary on stdout, and whether it ever reaches the transcript depends on the
   primary (an LLM) choosing to call `submit_review` with the text *verbatim*.
   It can forget the call, paraphrase, summarize, or editorialize. For a
   faithful second opinion, routing the content through the thing being
   second-guessed is the wrong shape — it is both an extra hop and a fidelity
   risk.

So `submit_review` gives a card to the case that doesn't need it and punts the
case that does.

### The surface already exists

ShipIt already emits a deterministic, persisted **`sub_agent_consult_card`**
("Consulted Codex · 47s") for every terminal status (§7). It is literally the
"here's what happened across the boundary" primitive — it just carries
*metadata* (status / duration / cost) and not the reviewer's *content*. And
`services/sub-agent.ts` already has the full result text (`result.text`) in
hand at emit time. So making cross-agent surfacing deterministic is a small,
...
- Add a content field (e.g. `outputMarkdown`) to `SubAgentConsultCard` — one
  ...
  card is already in `CARD_MESSAGE_FIELDS`, so the persistence path is built.
...
- Keep returning the text on stdout so the primary can still *act* on it (apply
  fixes) — surfacing and acting are decoupled.
...
styled card" — is a pure **presentation** concern (a cousin of `present`), not
a review- or spawn-specific one. If within-agent reviews genuinely want a
bordered card, that argues for a generic "render-as-card" primitive, not a
review-shaped MCP tool wired into the flow.

### The cost to accept (stated plainly)

Under this model an in-agent review looks like **prose** and a cross-agent
review looks like a **card**. That asymmetry is **information** — it signals
that an opinion crossed into a different model — not an inconsistency to be
smoothed away. The thing `docs/203` optimized for ("identical card in both
modes") is precisely what this trades away, on purpose. The open question to
settle before any code: do we accept that divergence (cross-agent is
deterministic because it's brokered; `Task` keeps narrating), or do we reframe
so the **consult card is always the review surface** and `submit_review` is
demoted to a presentation-only escape hatch?

### Reconciliation with `docs/203`

This partially supersedes `docs/203`'s "AI review flow": its §3 + "submit_review"
sections assume the parent records the card for both modes. If accepted, the
cross-agent branch of `docs/203` moves to the consult-card path here, and
`submit_review`'s role shrinks to (at most) the within-agent presentation case.
`docs/203` is currently implemented, so this is a deliberate revision of shipped
behavior, not a greenfield design — tracked on SHI-37, cross-referenced from
SHI-136.
