...
- [x] Serve `text/markdown` rendered to HTML (server-side via `marked`; user-facing tab still uses `react-markdown`)
...
- **Markdown fidelity** — the agent's screenshot view renders markdown with
  `marked`; the user's Present tab renders with `react-markdown`. Different
  renderers, acceptable for a screenshot-fidelity check (documented in
  `present-view.ts` and `present.md`).
