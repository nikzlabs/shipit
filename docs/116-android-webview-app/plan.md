...
2. **`MainActivity`** — the WebView host. On `onCreate`, reads the saved URL. If absent, launches `SettingsActivity` and finishes. Otherwise loads the URL into a full-bleed `WebView`. Configures: JS, DOM storage, third-party cookies (needed for the websocket session), `WebChromeClient.onShowFileChooser` for attachment uploads, hardware back button = `webview.goBack()` with fallback to default. There is **no toolbar** — the WebView is truly full-bleed. The two actions that used to live in a toolbar overflow are now chrome-free affordances: a small translucent **settings cog floating at top-center** (`@drawable/ic_settings`, ~32dp, `alpha 0.45`) where **tap → Settings** and **long-press → reload**, plus **pull-to-refresh** (an `androidx.swiperefreshlayout` wrapping the WebView) as the primary Reload gesture. Top-center placement deliberately avoids the corners, where ShipIt's own web UI keeps its controls.

> **On-device caveat:** ShipIt's web UI uses internal scroll containers, so the WebView document itself may sit at `scrollY == 0`, which can make `SwipeRefreshLayout` over-trigger on downward swipes. The cog's **long-press → reload** is wired up front as a belt-and-suspenders fallback, so if pull-to-refresh fights the chat scroll on real devices we can simply drop `SwipeRefreshLayout` without losing the Reload action. Flagged for the manual smoke test.
