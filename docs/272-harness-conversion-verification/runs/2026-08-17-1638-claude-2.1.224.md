---
issue: planning#430
title: Run — Claude Code 2.1.224 (2026-08-17)
description: PASS — first exercise of the recipe. Found two system subtypes falling through the adapter's bare default.
run:
  harness: claude
  cli: "@anthropic-ai/claude-code"
  cli-version: 2.1.224
  pin-at-run: 2.1.224 (docker/agent-cli/package.json — capture equals pin)
  started: 2026-08-17T16:38Z
  operator: agent (session "Run docs/272 recipe: Claude Code exercise")
  capture-model: claude-haiku-4-5-20251001 (direct CLI, subscription)
  live-model: deepseek-v4-flash (Claude harness redirected; metered ≈ $0.01)
  steps-run: 1, 2, 4, 5 (3 was a no-op — capture version equals the pin)
  capture-artifact: /persist/tour-claude/capture-2.1.224.ndjson (119 events, not committed)
  verdict: PASS
---

# Run — Claude Code 2.1.224 (2026-08-17)

First exercise of the [recipe](../plan.md). One file per run; the filename
carries the start timestamp, harness, and CLI version, and the `run:`
frontmatter block carries the metadata a later diff needs.

**Step 4 live run** used the dogfood inner instance with the Claude harness
redirected to DeepSeek — the seeded `anthropic:sub` route reproduced open
planning#358 (route reads `ready`, turn dies "not authenticated"), see
findings. The backing model does not change the event schema under test;
the CLI emits the same stream.
