...
- [x] ~~Keep the `.shipit/.env.<service-name>` fallback only for tests / no-root injection.~~ **Reversed by planning#292**: `serviceEnvDir` is now required and the in-workspace fallback (`writePerServiceEnvFiles`, `sweepWorkspaceServiceEnvFiles`, and the compose generator's `.shipit/.env.<svc>` default) is deleted. It was the last writer putting a ShipIt-generated file inside a user's clone — see `docs/246-shipit-state-out-of-clone/plan.md`.
...
- [x] `compose-generator.ts`: accept an optional `serviceEnvFiles?: Record<string, string>`
      ...
      `.shipit/.env.<service-name>` when absent~~ — **removed by planning#292**: a service
...
- [x] `compose-generator.test.ts`: override uses supplied absolute env-file paths, and (since
      planning#292) emits no `env_file:` for a service absent from the map.
