...
> per-vendor tabs, **D16**. Those rows are marked `✅ closed` below and the file paths in
> their evidence columns are pre-fix. `plan.md` → *"One card component — and no per-vendor
> tabs at all"* records what changed and why. Everything unmarked still stands.
