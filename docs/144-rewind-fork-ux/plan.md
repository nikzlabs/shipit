...
**Refinement (SHI-182, SHI-184) — fork is exempt from the running-turn gate.** B4's original "gate *all* handlers on `runner.running`" was too broad for `fork`. A fork spins off a **new** session from an already-committed SHA and never mutates the current one (its only parent-side effect is the breadcrumb append), so a running turn is no reason to block it — and the blanket gate stranded users trying to fork a session whose turn was hung. The server gate now reads `runner?.running && action !== "fork"`; in-place rewind (chat/code/both) stays gated. The client mirrors this: `RewindPoint` takes a `turnRunning` prop (instead of a blanket `disabled`) and, while a turn runs, presents the gap menu as **fork-only** rather than disabling the whole control. Two consequences follow: (1) the fork base is resolved to a concrete committed SHA up front (HEAD for a current-state fork, the most recent commit before the gap otherwise) so a concurrent end-of-turn auto-commit can't shift it under the `git clone --local`; and (2) `clearQueuedMessages` is skipped for fork — fork leaves the parent intact, so the parent's queued messages must survive. Separately (SHI-184), a session with **no auto-commits** has no per-message commit, so `findCommitBeforeGap` returns null — fork now falls back to HEAD instead of erroring with "No code state available to fork." Key file: `rollback-handlers.ts` (`handleRewindAtGap`, `action === "fork"` branch). Tests: `rewind-fork.test.ts` ("forks a session with no code commits…", "allows fork while a turn is running…"), `RewindPoint.test.tsx`.
