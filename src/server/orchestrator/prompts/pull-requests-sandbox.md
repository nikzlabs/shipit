...
- Never poll for a merge — no `while … gh pr view … sleep 60`. A blocking poll keeps the turn alive for hours, so the runner never goes idle and ShipIt can't reclaim the container. `shipit session notify-on-merge` isn't available in a sandbox, so just end your turn; the user picks it back up when the PR lands.
