 ...
 *
 * The name pattern excludes the two realistic false positives — a path-first
 * message (`/tmp/foo.ts is broken`) keeps a slash inside the token, and a
 * shell-variable mention (`$HOME is unset`) is uppercase. Every command a pinned
 * CLI lists, and every skill name, is lowercase.
 */
