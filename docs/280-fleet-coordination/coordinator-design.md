...
- UI for a months-long transcript (req 20, constrained by req 21): windowed rendering + lazy history paging for the coordinator view — on return the view opens on the *current* state (queue rail + latest exchanges), older history loads on scroll with scroll-position restore; ordinary date separators are the **only** visible seam. Mobile uses the same mechanism. (Benefits every long-lived session, but the coordinator makes it mandatory.)
