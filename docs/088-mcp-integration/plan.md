...
A row's key names the variable (or header) the MCP server reads, not the secret behind it, so renaming a key moves nothing: the row keeps its reference and its value. That is why a blank value can go on meaning "unchanged".

Two limits are accepted rather than fixed, both needing a config the panel cannot author — one that refers to a secret whose name is not its key's, or one that refers into another server's namespace. **Adding** a row whose key equals another row's secret name writes over that secret, because a new row's key is all there is to name its secret by. And a namespace freed by a rename and then taken by another server carries the new owner's values: the namespace follows the name, so a reference into a namespace whose owner has been renamed away is already dangling.
