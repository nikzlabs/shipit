...
describe("prepareSessionAgentEnvironment", () => {
  ...
  describe("a selection-less session is settled onto the install's first eligible model", () => {
    ...
    it("still writes when the derived service IS the native one but has no login (docs/272)", async () => {
      // OpenCode's native service is its own inference (Zen/Go) — pasted key,
      // no login flow. The native-vendor skip's premise ("the old unshaped
      // fallback works") is FALSE there: the OpenCode adapter refuses a spawn
      // with no routing. So a selection-less turn on an OpenCode-key-only
      // install must settle the row even though the derived service equals
      // `nativeServiceForHarness("opencode")`.
      const { state } = await prep({
        agent: "opencode",
        routes: [{
          id: "cred_oc", serviceId: "opencode", billingMode: "key", via: "string", label: "OpenCode",
          isPrimary: true, priority: 0, status: "ready", createdAt: 0, updatedAt: 0,
        }],
