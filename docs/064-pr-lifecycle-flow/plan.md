...
**Close pull request** appears in two places, gated so the user always sees exactly one. For the regular (mergeable) case it's an item inside the merge button's dropdown, separated from the merge methods and styled in `--color-error` — the most discoverable spot, right where the user already is. But the merge button only renders when the PR is mergeable (CI green, no conflicts, review satisfied), so for every state where merging isn't possible — most importantly **merge conflicts**, but also failing CI, a pending required review, or auto-merge armed — a standalone overflow menu (`ClosePrButton`, a kebab `⋮` trigger) carries the same action. `OpenPhase` and `PrStatusSection` render the kebab only when the merge button is hidden (`!showMergeButton`), so close is always reachable but never duplicated on screen. Folding close *only* into the merge dropdown was the original bug: a conflicted PR had no way to be closed from the card at all.

Both surfaces share one state machine and one rendered item so the destructive logic lives in exactly one place: `useClosePr(sessionId)` owns the two-step confirm (first click arms it → "Click again to confirm", so a misclick can't close a PR) plus the in-flight state and the actual `closePr` call; `ClosePrMenuItem` is the shared button. The second click calls `closePr` in `pr-store`, which `POST`s to the existing `/api/sessions/:id/pr/:number/close` route and optimistically flips the card to the `closed` phase. This keeps closing a PR inside ShipIt rather than bouncing the user to GitHub. Closed-but-not-merged PRs get a distinct red badge (`PrStateBadge`) using GitHub's `git-pull-request-closed` Octicon — `GitPullRequestClosedIcon`, a one-off custom SVG since Phosphor has no closed-PR glyph — so a closed PR no longer looks like a plain gray branch.
