...
- [ ] Verify: typecheck, lint:dev, co-located unit tests, browser check (HTML/SVG render + toggle incl. an SVG file from the tree, markdown frontmatter + review, Present absolute-path artifact read-only, dialog download of a binary/large file = raw bytes)
