...
export interface ImageAttachment {
   ...
   * of re-saving the image under a new randomized name — so a sent upload
   * stays as a single on-disk file (no `screenshot.png` + `screenshot-1.png`
   * duplication) and the `/uploads` panel keeps a stable path to preview it.
   */
  existingPath?: string;
}
