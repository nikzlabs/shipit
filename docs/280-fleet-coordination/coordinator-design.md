...
**Desktop dictation shortcut (req 32):** the docs/145 quick-capture overlay is the shape — global hotkey + dictation — retargeted at the coordinator instead of spawning a session; the coordinator decides what the dictated idea becomes (new session, message to existing work, a note). Reach beyond the focused browser app is a plan-phase scope question, staged like the phone app.
