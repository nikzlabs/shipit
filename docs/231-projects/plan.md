...
| Additional GitHub issue trackers | **Repo** | — | **No Project work needed.** Declared per repository in `shipit.yaml` (doc 247 req 5), so a Project's sessions already see only the trackers their own repositories declare. There is no deployment-wide binding to scope and no Default Project alias. Each declaration is additional to the code repository's own GitHub Issues; ShipIt's fixed public bug-report destination remains ShipIt-specific. Routing invariants: [doc 247](../247-private-github-issue-tracker/plan.md). |
