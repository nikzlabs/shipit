...
An audit, not a change. Nothing in product code was touched **when this was written**.

> **Status, 2026-08-11.** A follow-up pass closed **D1, D2, D5, D7, D8, D9** and, with the
> per-vendor tabs, **D16**. A second pass closed **D4, D10, D11**. **D3** was fixed in the same
> change that produced this audit and is closed too — the reveal state moved to `ui-store`, and
> the wiring survived the card refactor (`ServicesPanel.tsx:114`, `:206`). Those rows are marked
> `✅ closed` below and the file paths in their evidence columns are pre-fix. `plan.md` →
> *"One card component — and no per-vendor tabs at all"* records what changed and why.
>
> **Two rows are open, and they are open for different reasons.** **D6** (identify which secret a
> credential row holds) needs a server-side masked fingerprint on the wire shape and is simply not
> built. **D14** (failover cutoffs on a string-delivered subscription) is blocked on the
> `zai-plan-usage` quota reader, **planning#339**.
>
> **Three rows will never close, by design.** **D12** is a deliberate scope cut — there is no
> Settings → Harnesses screen and none should be built. **D13** describes the design working as
> intended. **D15** is the mockup being stale, not the code; the code states its reasoning in
> `UsageModal.tsx`. They are kept here as the record of a decision, not as outstanding work.
...
| Add-a-service dialog | inside `mock-services.png` | [`shipped-add-service-dialog.png`](./audit-shots/shipped-add-service-dialog.png), [`shipped-add-anthropic-sub-step3.png`](./audit-shots/shipped-add-anthropic-sub-step3.png); after D4: [`fix-add-anthropic-sub-step3.png`](./audit-shots/fix-add-anthropic-sub-step3.png) |
| Composer pickers | [`mock-picker-model-menu.png`](./audit-shots/mock-picker-model-menu.png), [`mock-picker-harness-menu.png`](./audit-shots/mock-picker-harness-menu.png) | [`shipped-picker-model-menu.png`](./audit-shots/shipped-picker-model-menu.png), [`shipped-picker-harness-menu.png`](./audit-shots/shipped-picker-harness-menu.png); after D10/D11: [`fix-picker-model-menu.png`](./audit-shots/fix-picker-model-menu.png), [`fix-picker-harness-menu.png`](./audit-shots/fix-picker-harness-menu.png) |
...
| D7 ✅ closed | Subscription card, routing controls | A visually separate shaded footer titled **HOW SHIPIT PICKS BETWEEN THESE ACCOUNTS**, holding the two radios and the cutoff row. | The two radios sit inline in the card body with **no section heading**, between the credential rows and the model chips. | (c) | `ServicesPanel.tsx:280-285`, `:375-391`. See `shipped-services.png` (GLM subscription card). | Small: a heading and a background band. |
| D8 ✅ closed **as (d)** | Subscription card with one credential | An explanatory strip: "One account — nothing to route between yet. Add a second to choose an order and a strategy." | Nothing. The routing block is simply absent. | **(d) — the mock is wrong** | `ServicesPanel.tsx` (`routedCredentials.length > 1`) | Closed the other way round. The strip was built, shown in the dogfood instance, and rejected on sight — "this is not needed, shouldn't be shown at all when there is only a single account" (2026-08-13 receipt in `requirements.md`). The cell's own caveat was the correct reading: `plan.md`'s argument against sentences explaining an absence applies here too, and the distinction drawn to keep it (a key card can *never* route, while this one names a reachable capability) is a carve-out req 19 does not contain. **The originally shipped behaviour was right.** Drop the strip from `mockup-services.html:456` if the mocks are to stay authoritative. |
| D9 ✅ closed | Every card header | Service avatar/initial badge; account-count pill (`2 accounts`). | Neither. | (c) | `ServicesPanel.tsx:238-255` | Small, cosmetic. |
| D10 ✅ closed | Model menu, group header | `SERVICE NAME` at left, billing mode as a **coloured pill** at the right edge (purple `Subscription`, green `API key`). Model rows are mono model ids. | Service name plus billing mode as **plain tertiary text** immediately after it; rows use friendly labels (`V4 Flash`, `Opus 5`), so the same model reads as `V4 Flash` under DeepSeek and `DeepSeek V4 Flash` under OpenRouter. | (c) | `ModelPicker.tsx:477-482` (label), `:497` (`row.label ?? formatModelName`). Compare `mock-picker-model-menu.png` / `shipped-picker-model-menu.png`. | Done — the pill is `BillingModePill`, the one Settings puts on a service card, so the two surfaces cannot drift. One trap the extraction walked into and a Codex review caught: the pill carries `normal-case tracking-normal` **itself**, because the model menu nests it inside `DropdownMenuLabel` (`uppercase tracking-wider`) and the old inline text reset that at the call site — without it the pill read `SUBSCRIPTION` in the menu and `Subscription` on the card, which is the drift it exists to stop. **The label inconsistency is deliberately still open**: the same model reads `V4 Flash` under DeepSeek and `DeepSeek V4 Flash` under OpenRouter, which is decided by each mode's `label` in `catalogue/services.ts`, not by the picker. Fixing it in the picker would paper over a catalogue question. |
| D11 ✅ closed | Harness menu row | Two lines — name, then `7 models available` beneath it. | One line — name, then `9 models` right-aligned; an uncredentialed harness reads `needs a credential` and is disabled. | (c), and the disabled-state copy is an improvement the mock lacks | `ModelPicker.tsx:212-243` | Done — name over count, `N models available`, matching what `ChoiceRow` already renders in the composer's settings menu. The `needs a credential` copy and the disabled row are kept. See `fix-picker-harness-menu.png`. |
