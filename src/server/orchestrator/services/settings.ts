...
function requireAccountService(provider: AgentId): string {
  ...
  // and it still has no accounts: the service is key-authenticated with no
  // login flow. So the gate is the login integration rather than the native
  // service alone, and every account verb on OpenCode stays a 400.
