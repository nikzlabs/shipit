...
- `npm test` — full suite. **The exception, not the habit** — CI runs it on every PR. See [Runtime](#runtime) for when it is warranted. Targeted run: `npx vitest run <file>.test.ts`.
