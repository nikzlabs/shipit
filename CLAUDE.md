...
**Checklist vs. issue — split by altitude, never mirror.** `checklist.md` and the tracked issue both describe "what's left," so it's tempting to keep the same list in both. Don't — that creates two sources of truth that drift, and there's then no single answer to "is this done?" Assign each surface the altitude it's good at:

- **The issue is the coarse unit of work plus its discussion and rationale** — why the work exists, the decisions and tradeoffs argued out async, status that automates on merge (`Closes`/`Refs`), and cross-PR/cross-branch coordination a repo file can't see. A tracker is a conversation medium; markdown is not.
- **`checklist.md` is the fine-grained implementation decomposition, coupled to the diff** — the mechanical steps you'd never file as tracker sub-tasks ("wire `toRow`/`fromRow`", "add the round-trip test"). Its value is that it's diffable, branch-scoped, and checked off in the *same* PR as the work, with no tracker round-trip — which is also why it deterministically drives the docs-list Active/Done grouping offline.

The test: if a checklist item could be copied verbatim into the issue as a sub-task, it's at the wrong altitude — it belongs to the issue. If it's a build step that only makes sense next to the code on this branch, it belongs in `checklist.md`. Keep rationale and discussion out of `checklist.md` (that's the issue's job) and keep granular build steps out of the issue (that's the checklist's job).
