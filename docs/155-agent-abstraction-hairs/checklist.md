...
- [x] ESLint rule flagging `agentId === "claude"` / `agentId === "codex"` outside `agents/<id>/` folders — added as `RESTRICTED_AGENT_ID_LEAK` in `eslint.config.js` via `no-restricted-syntax`. Two AST selectors cover both the `agentId === "x"` (Identifier) and `something.agentId === "x"` (MemberExpression) shapes, narrowed to identifiers whose name ends in `agentId`/`AgentId` so it doesn't fire on unrelated literal comparisons (DB row reads named `agent_id`, query strings named `agent`, runtime validators on `saved`/`provider`). Exemption blocks: `src/server/{session,orchestrator}/agents/{claude,codex}/**` (per-agent folders own their own dispatch) and `**/*.test.{ts,tsx}` (parameterized fixtures + SSE-event filtering assert per-agent behavior directly). Hair 9 (`agent-instructions.ts` parallel-sessions section dispatch) was finally completed as part of enabling the rule — replaced the inline if/else with a module-local `PARALLEL_SESSIONS_SECTIONS: Map<AgentId, string>` lookup. The remaining 15 sites are all annotated with `eslint-disable-next-line no-restricted-syntax -- <rationale>`: marketplace v1 gate (hair 7) in both server (`services/marketplace.ts`) + client (`SkillsTab.tsx`); Codex built-in skills merge (hair 8) in `api-routes-files.ts`; per-backend auth gates in `ws-handlers/send-message.ts` + `services/agent.ts` (candidates for future `Map<AgentId, AgentAuthManager>.isConfigured()` dispatch, deferred because per-backend refresh hooks and error copy still differ); Claude `--resume` JSONL recovery in `session-credentials.ts` (CLI-shape difference, plan non-goal); SSE auth event narrowing in `useServerEvents.ts` (runtime data routed into per-backend store slices); construction switch in `createWorkerAgent` (hair 11, legitimate per plan §11).
