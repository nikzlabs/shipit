...
**Corrected (planning#565).** Renaming was originally specified as delete-then-add, with a confirm prompt warning that the values must be re-entered. The prompt never shipped, and the edit form cannot re-enter them either: it blanks stored values and labels them "(unchanged)", so a rename submitted no replacement and the save silently destroyed the credentials. A save now *reconciles* the namespace instead — `updateMcpServer` keeps exactly the keys the new config refers to, taking each value from the submitted secrets, else from the same key under the old name (the rename carry-over), else from what is already stored, and clears everything else in either namespace. The old `mcp__<old>__*` keys are still reported as cleared, because they are: their values moved to `mcp__<new>__*`.

Two things that reconciliation depends on. References are read from **every** string in the config, not just the `env` / `headers` bag, because `args` resolves them too (`session/mcp-resolve.ts`) — a reference missed is a credential cleared. And the edit form re-submits the reference expression each row was loaded with, moving only the namespace on a rename, instead of re-deriving `$secret:mcp__<name>__<KEY>` from the key: re-deriving flattened a `Bearer ` prefix, a secret named unlike its key, and an OAuth `$platform:` link, and reconciliation would then clear what the config stopped referring to.

Renaming a *key* is not carried over — the store cannot tell it apart from removing one key and adding another — so the form drops the "(unchanged)" placeholder for a row whose key the user edited.
