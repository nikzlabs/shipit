...
Design: [`plan.md`](./plan.md). Build state: [`checklist.md`](./checklist.md).
...
- 2026-09-16 — Which model answers on camera, and with what credential? The agent had recommended recording with a real Anthropic key; Nik did not accept the arguments (he found the "240 x characters" DeepSeek reply weird, but not decisive) and instead connected his Claude **subscription** on the demo instance. So the demo session runs on the Claude subscription route: the chip reads "Opus 5", Opus records the takes, and no API key is involved anywhere — the proxy forwards the subscription's own bearer token in record mode (measured the same day: two requests on the bearer lane, both answered 200 by Anthropic through a base-URL-only redirect). The fake Anthropic key the agent had added was deleted. Requirement 6 stands; no new requirement.
