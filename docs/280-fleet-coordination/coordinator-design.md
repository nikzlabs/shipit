...
- UI for a months-long transcript (req 20): windowed rendering + lazy history paging for the coordinator view — load the recent window, fetch older on scroll; epoch boundaries render as collapsible day sections and give pagination natural seams. (Benefits every long-lived session, but the coordinator makes it mandatory.)
