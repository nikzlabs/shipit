...
| Repo | none — no clone, branch, PR, preview, auto-commit/push, warm pool, disk eviction |
...
- **Connect-through renders as two persisted cards**: a thin outgoing-relay line (destination, the user's verbatim words, receipt), and a source-reply card — the source agent's final text **verbatim** in a visually distinct untrusted frame, attributed by repo + session name, carrying the `messageId → turnId` correlation and an internal deep link. The same exchange lands in the source session's transcript with provenance; that remains the durable record.
...
**Structural injection containment (synthesis of Sol + Opus):** system-wake turns are **read-only** — write-capable tools (`message`, `spawn`, `answer`, `snooze`) are present only in turns that contain a current user utterance, and every write logs the triggering user-message id. `authorizedBy` on the API is the pre-registered hardening path.
