...
describe("Integration: PR creation", () => {
  ...
  /**
   * Helper: create a session so that git operations work (session isolation requires it).
   * Sends a message, waits for claude to start, finishes it, then returns.
   */
  async function createSession(client: TestClient) {
    ...
    // Drain messages until we're clear (session_started, claude_event, git_committed, etc.)
  ...
  /**
   * Helper: set up a session with a GitHub remote configured so PR creation works.
   */
