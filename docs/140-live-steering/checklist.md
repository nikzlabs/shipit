...
- [x] The three overlap hazards adoption creates, each pinned by a test that fails without its fix: `tryDrain` stands down when `turnIsCurrent()` is false; `agent_result` awaits an in-flight re-arm (`rearmInFlight`); the commit summary is snapshot at `agent_result` (`resultTurnSummary`)
...
- [x] Tests: `turn-self-wake-commit.test.ts` (adoption + commit; bare post-result init does NOT adopt; mid-turn and subagent output do not adopt; drain stands down; early result still runs its flow), `live-steering.test.ts` (busy session, clean accumulator, each turn persisted once)
