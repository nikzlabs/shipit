...
- [ ] Add one synchronous, fail-closed authorization check at the start of shared `dispatchOnRunner()`
- [ ] Resolve the authoritative remote from `sessionManager.get(runner.sessionId)` and check it with `RepoStore.isTrusted()`
- [ ] Deny unknown remotes, missing sessions, and missing production authorization dependencies
- [ ] Preserve trusted-by-construction behavior for sessions without a remote and ShipIt-created templates
- [ ] Ensure denial happens before settlement side effects, running-state mutation, steering, queue insertion, persistence, graduation, attachment reads, or process creation
- [ ] Re-check authorization when queued or recovered work re-enters `dispatchOnRunner()`
...
- [ ] Move the busy composer's direct live-steering path behind `runner.dispatch`
...
- [ ] Define one typed repository-trust admission failure shared by all transports
- [ ] Return HTTP `403` with stable machine code `repository_untrusted`
- [ ] Emit the same stable code and explanatory message on the WS error path
- [ ] Include authoritative session identity where supported
- [ ] Make a denied dispatch distinguishable from started, steered, and queued outcomes without parsing prose

## Client restricted state and consent UX

- [ ] Derive repository trust for normal and pre-first-turn warm routes from `currentSession?.remoteUrl ?? newSessionRepoUrl`
- [ ] Keep send disabled while a known remote's trust state is unresolved or explicitly untrusted
- [ ] Add repository trust to `MessageInput.disabled` without weakening existing connection-state disabling
...
- [ ] Keep the existing “Trust this repository” action as the only consent action
- [ ] Update the existing trust explanation to say agent messages, install, and services remain blocked until trust
- [ ] Enable messaging only after the authoritative trust response/SSE state update
- [ ] On trust-request failure, keep messaging disabled, reset the Trust button busy state with `try/finally`, and surface the error

## Optimistic-state rollback

- [ ] Give WS sends a precise request/delivery identity for correlating a rejected optimistic message
- [ ] Roll back only the matching optimistic WS or HTTP bubble on `repository_untrusted`
- [ ] Clear loading, activity, optimistic active-runner state, and an unsent pending WS frame for the rejected message
- [ ] Preserve the user's composer draft where practical
- [ ] Ensure rejection never leaves UI state implying that an agent turn started
...
- [ ] Assert an untrusted busy runner neither steers the resident agent nor grows its queue
