...
export async function handleRewindAtGap(ctx: RewindCtx, msg: WsRewindAtGap): Promise<void> {
  ...
  try {
    ...
    if (action === "chat") {
      ...
      ctx.send({ type: "rewind_complete", gapPosition, action: "chat", droppedMessageCount: removed.length });
      return;
    }
    ...
    if (action === "code") {
      ...
      ctx.chatHistoryManager.markRolledBackFromIndex(sessionId, gapPosition, rollbackHash);
      const replay = buildConversationReplay(allMessages);
      if (replay) ctx.sessionManager.setConversationReplay(sessionId, replay);
      ctx.sessionManager.clearAgentSessionId(sessionId);
      ctx.send({ type: "rewind_complete", gapPosition, action: "code", commitHash: rollbackHash });
      return;
    }
    ...
    if (action === "both") {
      ...
      if (replay) ctx.sessionManager.setConversationReplay(sessionId, replay);
      ctx.sessionManager.clearAgentSessionId(sessionId);
      ctx.send({ type: "rewind_complete", gapPosition, action: "both", droppedMessageCount: removed.length, commitHash: rollbackHash });
      return;
    }
