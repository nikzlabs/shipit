...
- [ ] Codex turns in local mode exit 1 after credentials link successfully. **Still open and still uninvestigated**, though `67480fd3` changed the Codex spawn env underneath it (one `codexConfigDir()` shared by the auth probe, the `config.toml` writer and the child; `HOME` + `CODEX_HOME` both set when scoped), so the failure should be re-observed before being diagnosed from the old symptom.
