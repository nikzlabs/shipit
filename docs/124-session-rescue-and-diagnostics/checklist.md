...
- [x] Gate `service_exited` on `shipit-service-name`, not on `shipit-parent-session` (§1.2a). The parent label is on every ShipIt-parented container, so egress-sidecar churn was reported as the project's services crashing; `serviceName` is now required and everything else emits `session_child_exited` (console only).
