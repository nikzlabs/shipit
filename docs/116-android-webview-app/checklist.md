...
- [x] `res/layout/activity_main.xml` — full-bleed `FrameLayout` → `WebView`, plus a top-center floating settings `ImageButton`; ShipIt-dark background. (Was CoordinatorLayout + AppBar + Toolbar — the toolbar/black bar was removed; the `SwipeRefreshLayout` wrapper was later removed too, see above.)
