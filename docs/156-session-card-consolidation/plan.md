...
> **Follow-up (auto-merge toggle surfacing):** the Auto-merge toggle no longer lives _only_ in the overflow. For an **open** PR it renders **inline** in the badge row (always visible on desktop + mobile — the prior `md:hidden` mobile-only gating is gone), since burying it in the overflow on desktop hurt discoverability (ShipIt principle: inline beats hidden). The redundant **"Will merge when CI passes"** status line under the row was removed — an enabled toggle next to the pending CI indicator already conveys that state. The overflow keeps a copy **only for non-open phases** (`phase !== "open"`: pre-PR / merged / closed) so auto-merge can still be armed before a PR exists; the toggle thus renders exactly once. Auto-fix CI remains a global setting (docs/169), not surfaced here.
