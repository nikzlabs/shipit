...
describe("credentialFailurePolicyFor — docs/252 req 12", () => {
  ...
  it("vendor-owned recovery needs account machinery, not just a native service (docs/272)", () => {
    ...
    // there is no OAuth healer and no sign-in a toast could start. A spent
    // OpenCode Go plan must take ShipIt's set-aside path — the same one GLM's
    // plan takes — or the dead credential is re-selected every turn.
    expect(credentialFailurePolicyForRoute("opencode", "sub", "opencode")).toMatchObject({
      stopsOnFailure: false,
      vendorOwnedRecovery: false,
    });
    // The login-backed natives keep their heal path exactly as before.
    ...
    // And an unpinned pre-feature row still answers TRUE (the no-op fallback).
