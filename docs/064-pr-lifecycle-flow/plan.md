...
**Close pull request** appears in two places so it's both discoverable in the common case and reachable when merging isn't possible. For the regular (mergeable) case it's an item inside the merge button's dropdown, separated from the merge methods and styled in `--color-error` — the most discoverable spot, right where the user already is. But the merge button only renders when the PR is mergeable (CI green, no conflicts, review satisfied), so for every state where merging isn't possible — most importantly **merge conflicts**, but also failing CI, a pending required review, or auto-merge armed — close also lives in the card's always-present PR actions menu (see below). Folding close *only* into the merge dropdown was the original bug: a conflicted PR had no way to be closed from the card at all.

### `PrActionsMenu` — the reusable PR-scoped overflow menu

The card's `⋮` overflow menu is `PrActionsMenu` (`PrActionsMenu.tsx`), a self-contained component that takes only a `sessionId` and derives everything else from the stores. It holds exactly the actions that operate on the *pull request / branch*:

- **Auto-merge toggle** (only for phases without an inline row — pre-PR, merged, closed; the open phase shows it inline on the card)
- **Sync with `<base>`** (remote-gated — `session.remoteUrl`)
- **Copy branch name**
- **Close pull request** (open phase only)

Because it's keyed only on `sessionId`, the *same* component is dropped verbatim into both surfaces: the inline `PrLifecycleCard` (top-right, next to the search icon) and the detail panel's **Status section action row** (`PrStatusSection`), right next to the merge / auto-merge / resolve-conflicts controls. It deliberately sits with the merge controls rather than in the detail-panel *header* — that's where users look for merge-related actions (so it's discoverable), and putting a `⋮` in the header threw the title/description edit pencils out of alignment. The trigger is always rendered so the menu is a stable home; in practice Copy branch name is essentially always available, so it's never empty for a real session. The destructive close still flows through the shared `useClosePr` hook + `ClosePrDropdownItem` (see above), and the menu's `onOpenChange` resets the armed close-confirm.

**Chat/session-scoped actions deliberately do NOT live in this menu.** "Download chat" and "Recover recent rewind" act on the *conversation*, not the PR, so they moved to the sidebar's per-session `⋮` menu (`SessionItem` in `SessionSidebar.tsx`). Both are self-contained there (reading `messages` / `rewindRecoveries` from the store; rewind dispatches the existing `shipit:restore-rewind` event bridged to the WS sender in `App.tsx`) and are shown **only for the active session's row** — `messages` only holds the current session's transcript, and a rewind restore must travel over that session's socket, so exposing them on an inactive row would download/restore the wrong session.
