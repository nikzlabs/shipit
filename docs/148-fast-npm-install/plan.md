...
- **Warm-pool pre-install (Option C below) is shipped** — `runPreInstall` is wired in [warm-pool-manager.ts](../../src/server/orchestrator/warm-pool-manager.ts). Gated on a standby container, which is itself gated on the existence of a container manager (i.e. *not* test mode or `RUNTIME_MODE=local`).
