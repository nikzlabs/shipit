...
Both schemes work **inside an artifact you presented** as well as in chat — a plain `<a href="shipit-preview://web/runs/1183">` in the HTML you wrote, or a markdown link in the `.md`, opens the destination on click. That is how an artifact becomes a control surface for the running app rather than a static picture of it.

Add the reserved parameter `shipit-render=badge` or `shipit-render=button` to render the pointer as an inline pill or a block button rather than a prose link: `[Open](shipit-present:/persist/reqs.html?shipit-render=button#req-7)`. ShipIt reads it on either side of the `#fragment` and strips it from both, so the page never sees it. An unopenable pointer stays clickable and explains itself in a toast. Full reference, including the exact markdown heading slug rules: `/shipit-docs/chat-links.md`.
