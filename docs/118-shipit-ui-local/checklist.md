...
- [ ] **The `shipit` MCP bridge stays absent in local mode ([SHI-303](https://linear.app/shipit-ai/issue/SHI-303)).** Every tool on it POSTs to the worker's `/agent-ops/*` — a host local mode does not have — so it is configured as `null` rather than advertised-and-broken (which would also point Claude's `--permission-prompt-tool` at an unreachable broker). So no `present` / `voice_note` / `propose_actions` / `report_shipit_bug` in dogfood turns. Same missing piece that leaves the image without the `gh` and `shipit` shims; one `/agent-ops` host closes both.
