...
- [x] Checkout mechanics: bare-cache reuse, per-generation checkout in the session state dir, install output confined to the generation directory (reqs 2, 7). **The writable layer is the generation directory itself** — a per-session, per-commit, disposable copy already gives install output a home that pollutes neither the shared cache nor the project, so the CoW layer buys nothing; read-only for the agent is enforced at the `:ro` mount
- [ ] Mounts: `/plugins/<name>` read-only into the agent container, plus `/project` for services (reqs 2, 21)
- [x] Generation staging/activation: stage → validate → install → atomic symlink swap; failure keeps the prior generation whole and live; per-repo in-flight serialization (reqs 12, 15). Service prepare/recreate lands with the compose work; old-generation semantics for concurrent CLI calls land with the CLI wrappers
...
- [x] Pin durability: resolved SHA persisted in `plugins/pins.json` keyed by the consumer declaration; a moved tag warns and does NOT move the plugin; only a declaration edit re-resolves (req 8)
- [x] Install stamping: re-runs on plugin commit / install string / `install-inputs` CONTENT change, mirroring agent.install's convention (req 7)
...
- [x] Snapshot + tab states: `active` (with the exact commit), `activating`, `degraded` (prior generation still live — req 15), `unavailable`; tracked-repo selectors now resolve against the LIVE generation's manifest (phase 2)
