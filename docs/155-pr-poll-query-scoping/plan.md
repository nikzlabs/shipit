...
- **GraphQL points cost of aliased calls.** Aliased `pullRequest(number: $n)` selections are billed per object, so cost should scale with the number of selections rather than `first: N`. Worth measuring against the current bulk-query cost on a real workload before celebrating the win.
