...
**The other exception — a mark you quote, not one you draw.** A **vendor logo** is an inline `<svg>` too, because Phosphor is a set of generic UI symbols and ships no brand marks. Nothing about a logo is a design decision: the shape belongs to its owner, so it is copied verbatim (Simple Icons is CC0 and is where ours come from) rather than approximated with a glyph. Draw it in `currentColor` so it survives every theme — a brand's own `#000` disappears on a dark background — and keep it `aria-hidden`, since the vendor's **name** is invariably beside it and a mark that announces itself makes a screen reader say the name twice. Current members: `ServiceLogo.tsx` (the service marks in Settings → Services, keyed on `ServiceId` with the service's initial as the fallback) and `SettingsIntegrations.tsx`'s `LinearLogo`. This exception is for logos only — it does not license a hand-drawn icon for anything ShipIt owns.
