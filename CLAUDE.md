...
**The inner ShipIt is seeded, and you can drive it yourself.** At `dev`-service boot a background step adds and trusts the repos listed in `scripts/dogfood-seed.json`, so the inner UI comes up with a repo ready to work in instead of an empty slate (`docs/131`). It skips repos already present, exits 0 on any failure, and honors `DOGFOOD_SEED=0`; its output is prefixed `[seed]` in the service logs. To then drive the inner ShipIt without clicking through its UI, `curl` its API — the `dev` service publishes 3000 and Vite proxies `/api` to the inner orch (resolve the host with `GET /api/sessions/${SHIPIT_SESSION_ID}/services` on the *outer* orchestrator, as in `shipit-docs/preview.md`). Four calls cover the loop: `GET /api/sessions/all` (what's there) · `POST /api/sessions/headless` `{repoUrl, initialPrompt}` or `POST /api/sessions/:id/agent/dispatch` `{text}` (start work — dispatch wakes a session that nobody has open) · `GET /api/sessions/:id/status` (still running?) · `GET /api/sessions/:id/history` (read the conversation back).
...
- **Get a different model's review before you hand work over.** Work done in this repo gets an independent review from the *other* backend — Claude-authored work is reviewed by Codex, Codex-authored work by Claude — via `shipit agent run --agent <other> --prompt-file -`. A same-model reviewer shares your blind spots; a different one doesn't, which is the whole point. **Use `shipit agent run`, not a subagent**: the Claude Code CLI carries a built-in "do not call the AgentTool unless the user requested it" instruction (it's in the CLI binary, not in this repo's config, so it can't be turned off here), and a brokered cross-agent run is not a subagent, so the rule doesn't apply to it. Practicalities: **you** judge when a result warrants review — a PR is the obvious trigger, but so is any substantive change, refactor, or design conclusion you're about to present as done; the review is **advisory**, so it never gates the PR and you decide which findings to act on; pass pointers not pastes (give the reviewer `git diff main...HEAD` to run itself, plus what to focus on); run it with `run_in_background: true` since a review-sized consult routinely outlives the 10-minute foreground cap, and recover a dropped one with `shipit agent result`. If the other backend isn't signed in, skip the review and say so — don't substitute a same-model stand-in. This is a **house rule for this repo only**, deliberately kept in `CLAUDE.md` and out of `prompts/` and `shipit-docs/`: ShipIt ships instructions to everyone's sessions, and our review workflow is not something to impose on people who build on ShipIt.
...
- **Doc has no `issue:` pointer.** Create an issue on the `roadmap` tracker (this repo's Linear team) to track it, then cross-link — all in the same turn (docs/187). Run `shipit issue create --tracker roadmap --title "<doc title>" --body-file - <<'EOF' … EOF`; the body should summarize the doc and link back to its path. **`--tracker <name>` is required** — `create` has no default destination precisely so a forgotten flag can't file into this public repository's own issues (docs/248); a bare `create` is rejected with the declared names listed. **Always set appropriate labels when creating an issue** (`--label <name>`, repeatable) so it's categorized correctly — pick the label(s) that match the work's intent (e.g. `feature`, `bug`, `chore`, `documentation`). The command prints the new issue's identifier and URL on stdout, so read the identifier and write the name form (`roadmap#SHI-NNN`) into the doc's `issue:` frontmatter in the same turn. Creation is do-then-surface — a provenance card with Undo (which cancels the issue) is posted automatically; you don't propose-and-wait. If the tracker isn't connected, the command says so — surface that to the user rather than filing somewhere else.
