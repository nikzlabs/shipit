...
Because it's keyed only on `sessionId`, the *same* component is dropped verbatim into both surfaces: the inline `PrLifecycleCard` (top-right, next to the search icon) and the detail-panel header (`PrDetailHeader`). The trigger is always rendered so the menu is a stable home; in practice Copy branch name is essentially always available, so it's never empty for a real session. The destructive close still flows through the shared `useClosePr` hook + `ClosePrDropdownItem` (see above), and the menu's `onOpenChange` resets the armed close-confirm.
