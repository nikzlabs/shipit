...
- **Auto-firing the rebase on poller detection.** We considered firing the rebase automatically when the poller observes `CONFLICTING`, with no button. Rejected: this can fire while the user is mid-thought on something else, and the rebase grabs the agent for an indeterminate number of turns. Manual trigger preserves user control. (Auto-trigger on push rejection is fine because the user just attempted to push; intent is fresh.) **Update (doc 146):** the auto-resolve-conflicts-on-idle feature deliberately reverses this decision under a default-off global setting (`autoResolveConflicts`). The new mitigations that make auto-fire acceptable — an idle gate so the rebase never grabs the agent mid-turn, a 3-attempt cap with cooldown, and a failure banner only on terminal exhaustion — are documented in `docs/146-auto-resolve-conflicts-on-idle/plan.md`.
