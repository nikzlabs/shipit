...
- [ ] Local-mode workspace trust: the CLI drops `permissions.allow` entries because `CLAUDE_PRE_TRUSTED_DIRS` covers `/workspace` but an inner session lives at `/workspace/sessions/<id>/workspace`. **Still open** — neither commit touched it. Cosmetic-ish (permission prompts, not a dead turn), pre-existing.
...
- [ ] Local-mode agents get no MCP at all — no Playwright/browser server, no ShipIt bridge tools, no user MCP servers, for either backend. Tracked as [SHI-298](https://linear.app/shipit-ai/issue/SHI-298); blocks the last docs/244 verification (a real MCP image-block result) and any other MCP-shaped dogfood check. **Two independent container gates, both of which have to be closed** — fixing either alone leaves MCP broken, as config with no credentials or credentials with nothing configured:
  - *Config* — `invokeAgentMcpWriter()` is called only by `AgentController` (`agent-controller.ts:134`), and `McpConfigController` is constructed only by `session-worker.ts:157`. Local mode has no session worker, and its plain `SessionRunner` has no equivalent MCP-writing path.
  - *Credentials* — the merged agent-env push (step 4 of `prepareSessionAgentEnvironment`, feeding `collectMcpAgentEnv`'s `mcp__*` / `MCP_PLATFORM_*` entries) is gated on `runner instanceof ContainerSessionRunner` at `session-agent-env.ts:626`.

  Same structural shape as SHI-282, so likely the same fix shape: do it at the spawn, where `local-agent-home.ts` already resolves HOME, rather than un-gating env prep. Playwright is additionally a *binary* dependency — `Dockerfile.dogfood` puts `playwright-mcp` on PATH, but Chromium's system libraries are a separate question. Cover both Claude and Codex; do not infer coverage from the containerized worker path.
...
- [x] The operational half of the env-var story, confirmed the hard way: with `ANTHROPIC_AUTH_TOKEN` set on the `dev` service, turns failed with `Agent CLI requires authentication` — a symptom identical to SHI-282 and *not* it. Unsetting it in outer Settings → Secrets is what let the first turn through. `scrubEnvAuthForScopedHome` means that is no longer required, but the incident is why the docs still say to leave it unset.
  (Inner sessions having no MCP servers — first recorded here from the missing-browser-tool symptom — is the same gap; consolidated into the SHI-298 entry under **Follow-up** above so there is one record, not two.)
