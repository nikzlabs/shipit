...
- [x] Codex turns in local mode no longer reproduce the recorded exit 1 on the current spawn path (Codex CLI 0.146.0). Drove the real binary with subscription file auth and the adapter's exact scoped environment — `HOME=<account-root>`, `CODEX_HOME=<account-root>/.codex`, and `OPENAI_API_KEY` absent — and `codex exec` completed a model turn with exit 0. The `HOME`-only and `CODEX_HOME`-only controls also completed with exit 0. The negative control reproduced exit 1 only when `CODEX_HOME` named a directory that did not exist (`Error finding codex home`); that state is not reachable through the current adapter because `writeMcpConfig()` creates the same `codexConfigDir()` before `run()` spawns the child. The dogfood service itself booted successfully in `RUNTIME_MODE=local`, but its fresh inner state had no Codex provider account (`authConfigured: false`), so it could not supply an additional UI-driven turn without interactive sign-in. No speculative code change: `67480fd3`'s shared config-dir resolution and scoped `HOME` + `CODEX_HOME` have removed the only directly reproducible failure shape.
