...
function subtitle(action: RewindGapAction, preview?: WsRewindPreview): string {
  if (action === "fork") {
    if (preview?.keptTurnGroupCount !== undefined) {
      return `Includes ${plural(preview.keptTurnGroupCount, "turn group")}`;
    }
    return "New session from this point";
  }

  const turnText = preview?.discardedTurnGroupCount !== undefined
    ? plural(preview.discardedTurnGroupCount, "turn group")
    : "later turns";

  if (action === "chat") return `Discard ${turnText}`;

  const fileText = preview?.fileCount !== undefined
    ? plural(preview.fileCount, "file")
    : "changed files";
  if (action === "code") return `Reset ${fileText}, keep chat`;
  return `Discard ${turnText} and reset ${fileText}`;
}
...
export function RewindPoint({
  ...
  const modalSummary = useMemo(() => {
    ...
    return subtitle(pendingAction, modalPreview);
  ...
  return (
    <div className="group/rewind flex items-center gap-3 py-1" data-testid="rewind-point">
      <div className="h-px flex-1 bg-(--color-border-secondary)" />
      <DropdownMenu onOpenChange={requestPreviews}>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            disabled={disabled}
            className="inline-flex h-7 items-center gap-1 rounded-md border border-(--color-border-secondary) bg-(--color-bg-secondary) px-2 text-xs font-medium text-(--color-text-secondary) opacity-0 transition-opacity hover:bg-(--color-bg-hover) hover:text-(--color-text-primary) focus:opacity-100 focus:outline-none disabled:cursor-not-allowed disabled:opacity-40 group-hover/rewind:opacity-100"
        ...
        <DropdownMenuContent align="center" side="top" className="w-64">
          {!currentState && (
            <>
              <DropdownMenuItem onSelect={() => chooseAction("chat")} className="flex-col items-start">
                ...
                <div className="mt-0.5 text-(--color-text-secondary)">{subtitle("chat", previews?.chat)}</div>
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => chooseAction("code")} className="flex-col items-start">
                ...
                <div className="mt-0.5 text-(--color-text-secondary)">{subtitle("code", previews?.code)}</div>
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => chooseAction("both")} className="flex-col items-start">
                ...
                <div className="mt-0.5 text-(--color-text-secondary)">{subtitle("both", previews?.both)}</div>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          )}
          <DropdownMenuItem onSelect={() => chooseAction("fork")} className="flex-col items-start">
            ...
            <div className="mt-0.5 text-(--color-text-secondary)">{subtitle("fork", previews?.fork)}</div>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      <div className="h-px flex-1 bg-(--color-border-secondary)" />
