...
- [x] Gate the edge on the new `AgentCapabilities.startsOwnTurns` capability (Claude true, Codex false), resolved through the agent registry — NOT on `useStreaming`: Codex streams but emits its turn's final assistant text after `turn/completed`, so there the adoption shape means the opposite
- [x] The five overlap hazards adoption creates, each pinned by a test that fails without its fix: `tryDrain` stands down when `turnIsCurrent()` is false, and re-checks after the queued-turn commit; `agent_result`, the adapter-`error` path and `done` all await an in-flight re-arm (`rearmInFlight`, non-null only while one is running); the commit summary falls back to the `agent_result` snapshot only when the runner has moved on
