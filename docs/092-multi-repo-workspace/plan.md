...
- The kebab menu on each repo header has three items: **View All Sessions** (opens AllSessionsDialog filtered to that repo), **Project Settings** (opens the per-repo `ProjectSettings` dialog scoped to that repo — deployments + secrets), and **Remove Repository** (destructive, uses a "Click again to confirm" inline guard — see `Settings.tsx` for the same idiom). Originally specced as a right-click context menu with three items (remove repo, copy URL, open on GitHub); shipped as a left-click kebab, since "View All" wanted a permanent home and right-click is not discoverable. Copy URL and Open on GitHub are deferred — neither is needed inline once the repo's sessions are already in the sidebar.
  - **Project Settings split (per-repo dialog):** deployments and secrets are *project*-scoped, not workspace-scoped, so they live in their own `ProjectSettings.tsx` dialog reached from this menu rather than as tabs in the workspace-wide `Settings.tsx`. Open state is `ui-store.projectSettingsRepoUrl` (the repo URL, or `null` when closed) plus `projectSettingsTab` for deep-linking; `setProjectSettingsRepoUrl(url, tab?)` opens it. The preview pane's "missing required secrets" banner (`PreviewFrame.tsx`) opens this dialog straight to the Secrets tab for the active session's repo.
