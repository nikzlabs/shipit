...
[requirements.md](./requirements.md). In particular, additional GitHub issue
trackers are declared per repository rather than bound per Project, so they need
no phase-1c work; see
[doc 248](../248-declared-issue-trackers/requirements.md).
...
| Additional GitHub issue trackers | **Repo** | — | **No Project work needed.** Declared per repository in `shipit.yaml` (doc 248 req 5), so a Project's sessions already see only the trackers their own repositories declare. There is no deployment-wide binding to scope and no Default Project alias. Each declaration is additional to the code repository's own GitHub Issues, and ShipIt's public bug-report destination is unaffected. Routing invariants: [doc 248](../248-declared-issue-trackers/plan.md). |
...
- `api-routes-session-repos.ts` (`/api/repos/trust` fan-out), `service-manager-setup.ts`
  — trust gate sites
- `trackers/registry.ts`, `services/issues.ts` — Linear per Project (declared
  GitHub trackers are repo-scoped already, so they need no change here)
