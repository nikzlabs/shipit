...
- [x] Handle dispose (defined drop/preserve for buffered-but-unsent steer) / `restartAgent` (docs/127 — drops in-flight turn, resumes cold via `--resume`) / agent-container OOM (OOM circuit breaker, docs/122/124 — **not** docs/126, which is preview-OOM) / reconnection with `--resume`-on-respawn. **Resolved + documented** in plan.md "Lifecycle semantics — resolved": dispose kills the agent, clears the queue + turn-event buffer, and resets `isStreamingActive` (buffered steer is dropped, not delivered to a dying process — covered by `session-runner.test.ts` "dispose tears down a streaming turn …"); `restartAgent`/OOM/orchestrator-restart all recover via re-spawn + `--resume` (the `done`/exit path in `turn-executor.ts` already clears the gate so respawns take the fresh-spawn path, never closed stdin). Disconnect/reconnect, session-switch-targeting, and interrupt-during-steer have dedicated integration coverage in the Phase 5 resilience suite.
...
- [x] Integration tests for: steer-then-disconnect/reconnect; steer-during-session-switch targets correct runner; interrupt during steered turn (`live-steering-resilience.test.ts`)
...
- [x] **Validate in production** that a gap-steer is genuinely un-echoed (the fix's assumption). **Done 2026-08-13 — half of it does not hold**, and the escalation lever written here was the wrong fix. The CLI *does* echo a steer it received but did not apply to the finishing turn: it accepts the message and runs it as a turn **of its own** after the `result` (production trace in plan.md "Phase 6.11"). Dropping the echo from the delivered predicate would therefore double-process the message, not rescue it. The re-queue predicate is unchanged; what was missing is that the orchestrator never learned the CLI-started turn existed — fixed by adopting it when that turn produces its first top-level output.

## Phase 6.11 — adopt a turn the CLI starts on its own (docs/140 plan.md "Phase 6.11")

- [x] `adoptCliStartedTurn` in `agent-listeners.ts`: one helper behind both edges (`agent_self_wake` and top-level assistant output after this turn's `result`), reusing the docs/235 machinery — `resetRunnerTurnState` (only when no turn is in flight) + `running = true` + `session_status running:true`
- [x] `sawTurnResult` on the wired listeners as the discriminator: output before this turn's `result` is just this turn talking, and a backgrounded subagent's output carries `parentToolUseId`
- [x] **Not** the `agent_init`, despite being the obvious announcement: the CLI emits one for `set_permission_mode` with no turn behind it, and adopting that wedges the session as permanently busy (plan.md "Phase 6.11" — reviewer finding, docs/261)
- [x] `rearmForCliStartedTurn` in `turn-executor.ts` (renamed from `rearmForSelfWokenTurn`) fires on the same edges, so the adopted turn gets its own auto-commit, push, drain and PR card
- [x] Gate the edge on the new `AgentCapabilities.startsOwnTurns` capability (Claude true, Codex false), DERIVED inside `executeAgentTurn` from `agentId` rather than threaded through each entry point — the WS path had it and `runDispatchedTurn` (which also streams) did not, leaving the original bug live on a supported path. NOT `useStreaming`: Codex streams but emits its turn's final assistant text after `turn/completed`, so there the adoption shape means the opposite
- [x] The overlap hazards adoption creates, each pinned by a test that fails without its fix: `tryDrain` stands down when `turnIsCurrent()` is false, and re-checks after the queued-turn commit; `agent_result`, the adapter-`error` path and `done` all await an in-flight re-arm (`rearmInFlight`, non-null only while one is running); the commit summary falls back to the `agent_result` snapshot only when the runner has moved on; an adopted turn that dies without a result still gets its rows finalized
- [x] The **failed auth heal** — the fourth terminal path — awaits the re-arm too
- [x] An adopted turn is never RE-DISPATCHED (`servingAdoptedTurn`): the quota failover and auth-heal retry would re-run the *user's* prompt, not the adopted turn's. **Not pinned by a test** — see plan.md for why (needs the credential-selection harness on a streaming dispatched turn); reasoned from the code and stated rather than implied
...
- [x] Tests: `turn-self-wake-commit.test.ts` (adoption + commit; bare post-result init does NOT adopt; a backend that cannot start its own turns does NOT adopt; mid-turn and subagent output do not adopt; drain stands down; drain re-checks after the commit; early result and a crash mid-re-arm both still run their flow), `live-steering.test.ts` (busy session, clean accumulator, each turn persisted once)
...
- [x] Update `src/server/shipit-docs/` if any agent-facing behavior changes — **confirmed no change needed**: live steering is a user→agent transport concern (when a user message folds into a running turn), not a documented platform contract the in-container agent relies on. The agent-facing docs (preview, `shipit.yaml`, container environment, GitHub) are unchanged.
- [x] Decide `result subtype` taxonomy: normal vs interrupted vs error for post-turn flow (auto-commit, PR card) — **resolved** (plan.md "Open questions"): keep shipped behavior — `subtype:"success"` is a normal turn-end and runs the full post-turn flow on `agent_result`; `error` / `error_during_execution` (interrupted streaming turn) routes to the error handler, skips commit/PR side effects, still flips `running=false` and persists the partial turn. No new subtype gating warranted.
- [x] Decide whether to adopt `--include-partial-messages` (token-level streaming) or keep message-granularity — **resolved (deferred)**: keep message-granularity; the steering/echo-dedupe/persistence contracts are message-boundary-based and token partials would widen blast radius for a cosmetic win. Revisit additively if finer streaming is requested.
