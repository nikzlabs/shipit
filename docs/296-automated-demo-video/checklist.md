...
- [x] `proxy.mjs` — lanes by auth header kind; record mode (forward, swap `x-api-key` for the proxy's key, save `<lane>/<n>.sse` + fingerprints); replay mode (nth response per lane, paced SSE, drift log, `HEAD /api/hello` → 200, exhausted cassette → 400, non-retryable); 27 tests
...
- [x] `cut.sh` — keep `[actionAt, +lead]` and `[readyAt, +hold]` per beat, concatenate, export `hero.mp4` (h264, yuv420p, faststart, no audio) and `hero.webm` (VP9, no audio); `cut-plan.mjs` re-anchors the beats on the video's real duration; verified on run 8 (65 s → 18.56 s, exactly the plan)
