...
| `checkout.writable` | this surface's own mount — not the self/consumer signal (read `mode`), but on the CLI surface it checks the rule directly: `/plugin` is writable exactly when it is the project, so a consumer generation is `false` and `repo: self` is `true` (reqs 7, 15, 27). On the service surface it measures this fragment's own `.:/app:ro`, not ShipIt's `/plugin` |
