...
- Context lifecycle — **decided 2026-08-23: v1 relies on the harness's built-in auto-compaction** ("it's kind of free — see how it goes"). What stands regardless: transcript ≠ context; context is a cache over three durable stores; whatever happens is invisible (req 21); the calendar has no role.
...
4. *(Resolved 2026-08-23 in two steps — final model: **one delivery surface at a time**, reqs 22–26.)* ShipIt derives **where the user is** and routes unsolicited delivery there and nowhere else:
   - **Explicit hands-free mode wins over everything, stickily.** While on — regardless of what the user does on a visible desktop — arrivals are spoken through the phone app (lock screen included), one item per speaking turn (req 19), and no other surface pings. It ends only by explicit toggle, never because a desktop window got focus.
...
**Outbound webhooks are back in v1 — the "zero external consumers" premise fell (2026-08-23, reqs 23–25).** The hands-free channel is a phone app speaking coordinator items (payload: the coordinator's spoken text plus correlation ids — still no untrusted-prose dumps) and POSTing voice replies back as coordinator messages with a scoped token. **The end state is a first-party ShipIt companion app (req 25)** — "a super smooth experience is only achievable with a custom app"; the user's existing private webhook app is the interim workaround and proves the contract. Same contract either way, so the workaround-to-first-party transition changes no API. One manually-minted token suffices until then; the token-minting *Settings UI* stays deferred. The app is a thin delivery client — no coordinator logic leaves ShipIt.
