...
**1. Propose first — never tag without confirmation.** Read the version source (for a Node repo, the \`version\` field in \`package.json\`), compute the next [semver](https://semver.org) for the requested bump (patch / minor / major, or the explicit version the user named), and **propose** it. Do NOT bump, commit, tag, or push yet. Emit a proposal marker on its own line so ShipIt shows the confirmation card:
